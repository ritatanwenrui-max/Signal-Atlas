...
function localizedPlatformName(platform: string, uiLanguage: UiLanguage) {
  return uiLanguage === "en" && platform === "网页新闻" ? "Web news" : platform;
}
...
function CoverageView({ connectors, sources, diagnostics, submit, canManage }: { connectors: Connector[]; sources: MediaSource[]; diagnostics: CollectionDiagnostic[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canManage: boolean }) {
  ...
  return <div className="coverage-page">
    <section className="coverage-architecture panel-dark"><div><h2>采集计划与混合分析</h2><p>新闻档案优先使用多个独立新闻源和社交搜索索引发现新内容；搜索与归档确认没有新增后，后台才进入帖子评论、回复和受众分析阶段。</p></div><div className="architecture-flow"><article><b>01</b><strong>全球新闻发现</strong><span>NewsAPI.ai / NewsData / The News API / GNews / Guardian / GDELT 等</span><small>按各自额度分时运行</small></article><i>→</i><article><b>02</b><strong>社交内容补漏</strong><span>Bluesky / Mastodon / Hacker News / ScrapeCreators / Brave 等</span><small>平台搜索与多索引交叉发现</small></article><i>→</i><article><b>03</b><strong>规则全量分析</strong><span>品牌限定 / 排除词 / 地区 / 事件</span><small>人工标注优先</small></article><i>→</i><article><b>04</b><strong>受众采集</strong><span>评论 / 回复 / 翻译 / 语义</span><small>档案阶段完成后运行</small></article></div></section>
