...
import { getUiLocale, type UiLanguage, useAmericanEnglishBatch, useInterfaceLanguage } from "./ui-language";
...
type Mention = {
  ...
  capture_status?: string; capture_error?: string; capture_updated_at?: string;
...
type Connector = { id: string; name: string; provider?: string; configurable?: boolean; configured?: boolean; lastFour?: string; status: "online" | "limited" | "credentials" | "approval"; detail: string; retryAt?: string; pending?: number; lastError?: string;
  quotaUsed?: number; quotaLimit?: number; quotaRemaining?: number; quotaResetAt?: string; scheduleLabel?: string; stateLabel?: string };
...
type SearchDemandSignal = { signal_date: string; clicks: number; impressions: number; ctr_micros: number; position_millis: number; complete: number; source: string; collected_at: string };
type EventOrigin = { id: number; event_key: string; title: string; url: string; platform: string; source: string; source_country: string; published_at: string; note: string; active: number };
type SearchEvent = { id: number; event_key: string; peak_date: string; start_date: string; end_date: string; peak_clicks: number; peak_impressions: number; baseline_clicks: number; spike_ratio: number; trigger_source: string; status: string; updated_at: string; origin: EventOrigin | null };
type DashboardData = {
  ...
  searchDemandSignals: SearchDemandSignal[]; searchEvents: SearchEvent[];
...
type StoryCluster = { key: string; items: Mention[]; title: string; risk: number; impact: number; countries: string[]; platforms: string[]; latest: string; originCountry: string; originSource: string; signal: SearchEvent; origin: EventOrigin | null };
...
const emptyData: DashboardData = { mentions: [], entities: [], alerts: [], syncRuns: [], collectionDiagnostics: [], searchDemandSignals: [], searchEvents: [], brand: null, connectors: [], mediaSources: [], propagationEdges: [], analytics: emptyAnalytics, syncPipeline: null, redditSyncPipeline: null, aiBrief: null, viewer: { authenticated: false }, workspace: null };
const nav = [
...
const platformCatalog = ["网页新闻", "博客", "Instagram", "Facebook", "TikTok", "X", "YouTube", "Reddit"] as const;
const platformVisuals = [["网页新闻", "web"], ["博客", "blog"], ["Instagram", "instagram"], ["Facebook", "facebook"], ["TikTok", "tiktok"], ["X", "x"], ["YouTube", "youtube"], ["Reddit", "reddit"]] as const;
function platformSlug(value: string) { return platformVisuals.find(([label]) => label === value)?.[1] ?? "other"; }

function eventMatchTokens(value: string) {
  return new Set((value.toLowerCase().match(/[\p{L}\p{N}]{2,}/gu) ?? []).filter((token) => !["the", "and", "with", "from", "this", "that", "official", "brand", "事件", "官方", "品牌"].includes(token)));
}
...
const pipelineStageLabel: Record<SyncPipelineJob["stage"], string> = {
  reddit: "Reddit 独立发现", maintenance: "档案维护", discovery: "新闻档案优先采集", audience: "按需评论任务与分析",
};
...
function translatedStoredTitle(original: string, translation: string | undefined, sourceLanguage: string | undefined, uiLanguage: UiLanguage, fallback = "Archived social post") {
  if (uiLanguage !== "en" || sourceIsEnglish(sourceLanguage)) return original;
  return translation?.trim().split(/\n+/)[0]?.trim() || fallback;
}

type TranslatableMention = Pick<Mention, "title" | "excerpt" | "summary" | "translation_en" | "language" | "source">;

function mentionTranslationSource(item: TranslatableMention) {
  return [item.title, item.excerpt || item.summary].filter(Boolean).join("\n").slice(0, 2_000);
}

function translatedMentionCopy(item: TranslatableMention, uiLanguage: UiLanguage) {
  ...
  if (!translated) return { title: `${item.source || "Archived source"} coverage`, excerpt: "Open the original source for the full text." };
  const [title, ...excerpt] = translated.split(/\n+/).map((part) => part.trim()).filter(Boolean);
  return { title: title || translated, excerpt: excerpt.join(" ") || "" };
}

function useMentionCopies(items: Mention[], uiLanguage: UiLanguage) {
  const translationSources = useMemo(() => items.filter((item) => uiLanguage === "en" && !sourceIsEnglish(item.language) && !item.translation_en?.trim()).map(mentionTranslationSource), [items, uiLanguage]);
  const translations = useAmericanEnglishBatch(translationSources, uiLanguage);
  return (item: Mention) => {
    const source = mentionTranslationSource(item);
    return translatedMentionCopy({ ...item, translation_en: item.translation_en?.trim() || translations.values[source] || "" }, uiLanguage);
  };
}
...
function translatedCommentCopy(comment: Pick<SocialCommentRow, "content" | "translation_en" | "language">, uiLanguage: UiLanguage) {
  ...
  return comment.translation_en?.trim() || "Comment text is available from the original source.";
}

function localizedCountryName(country: string, uiLanguage: UiLanguage) {
  if (uiLanguage !== "en") return country;
  const code = countryCode[country];
  if (!code || ["??", "GL", "ZH"].includes(code)) return country === "全球" ? "Global" : "Unknown region";
  try { return new Intl.DisplayNames(["en-US"], { type: "region" }).of(code) || country; }
  catch { return country; }
}

function localizedPlatformName(platform: string, uiLanguage: UiLanguage) {
  return uiLanguage === "en" && platform === "网页新闻" ? "Web news" : uiLanguage === "en" && platform === "博客" ? "Blogs" : platform;
}
...
export default function Home() {
      ...
      for (let attempt = 0; attempt < 90; attempt += 1) {
        ...
        if (job.status === "completed") {
          if (announce) {
            ...
            setToast(`主巡检完成，新闻档案与内容分析已更新${redditNote}`);
  ...
  async function collectMentionComments(mentionId: number) {
    const response = await fetch("/api/comments", { method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "collectMention", mentionId }) });
    const result = await response.json();
    if (!response.ok) {
      const message = result.error ?? "评论采集未能启动";
      setToast(message); window.setTimeout(() => setToast(""), 4500);
      throw new Error(message);
    }
    const refreshed = await fetch("/api/data");
    if (refreshed.ok) setData(await refreshed.json());
    setToast(result.analyzedComments != null ? `已取得并分析 ${result.analyzedComments} 条公开评论` : "评论采集与情绪分析已手动开启");
  ...
  const clusters = useMemo(() => {
    return data.searchEvents.flatMap((signal): StoryCluster[] => {
      const windowItems = filteredMentions.filter((mention) => {
        const day = mention.published_at.slice(0, 10);
        return day >= signal.start_date && day <= signal.end_date;
      });
      const origin = signal.origin;
      const grouped = new Map<string, Mention[]>();
      for (const mention of windowItems) grouped.set(mention.cluster_key, [...(grouped.get(mention.cluster_key) ?? []), mention]);
      const originTokens = eventMatchTokens(`${origin?.title ?? ""} ${origin?.note ?? ""}`);
      const rankedGroups = [...grouped.values()].map((groupItems) => {
        const items = [...groupItems].sort((a, b) => a.published_at.localeCompare(b.published_at));
        const ids = new Set(items.map((item) => item.id));
        const inheritedEdges = data.propagationEdges.filter((edge) => ids.has(edge.from_mention_id) && ids.has(edge.to_mention_id)).length;
        const groupTokens = eventMatchTokens(items.map((item) => `${item.title} ${item.excerpt}`).join(" "));
        const originOverlap = [...originTokens].filter((token) => groupTokens.has(token)).length;
        const hoursFromRise = Math.max(0, (new Date(items[0]?.published_at ?? `${signal.start_date}T00:00:00Z`).getTime() - new Date(`${signal.start_date}T00:00:00Z`).getTime()) / 3600_000);
        return { items, score: items.length * 30 + inheritedEdges * 18 + originOverlap * 12 - Math.min(30, hoursFromRise / 8) };
      }).sort((a, b) => b.score - a.score || b.items.length - a.items.length);
      // Search demand defines when an event starts; the existing text-similarity cluster decides which coverage belongs to it.
      const archiveItems = [...(rankedGroups[0]?.items ?? [])].sort((a, b) => a.published_at.localeCompare(b.published_at));
      const originNode: Mention | null = origin ? {
        id: -Math.abs(origin.id), title: origin.title, url: origin.url, source: origin.source, platform: origin.platform,
        source_country: origin.source_country, content_country: origin.source_country, language: "英文", sentiment: "中性", emotion: "官方发布",
        risk: 18, impact: 90, summary: origin.note, cluster_key: signal.event_key, parent_url: "", relation: "事件首发锚点", engagement: 0,
        excerpt: origin.note, author: origin.source, provider: "Official origin", discovered_via: "event_origin", translation_en: "", translation_status: "skipped",
        translation_provider: "", translation_error: "", translation_attempts: 0, translation_next_retry_at: "", translated_at: "", content_hash: "",
        word_count: 0, sentiment_score: 0, topics: "事件起点", keywords: "", first_seen_at: origin.published_at, archived_at: origin.published_at,
        published_at: origin.published_at, location_confidence: 100, location_method: "人工确认", event_origin: true,
      } : null;
      const ordered = [...(originNode ? [originNode] : []), ...archiveItems].sort((a, b) => a.published_at.localeCompare(b.published_at));
      if (!ordered.length) return [];
      return [{ key: signal.event_key, items: ordered, title: origin?.title ?? ordered[0].title,
        risk: Math.max(18, ...archiveItems.map((item) => item.risk)), impact: Math.max(50, ...archiveItems.map((item) => item.impact)),
        countries: [...new Set(ordered.map((item) => item.source_country))], platforms: [...new Set(ordered.map((item) => item.platform))],
        latest: ordered.at(-1)?.published_at ?? signal.end_date, originCountry: origin?.source_country ?? ordered[0].source_country,
        originSource: origin?.source ?? ordered[0].source, signal, origin }];
    }).sort((a, b) => b.signal.peak_date.localeCompare(a.signal.peak_date));
  }, [filteredMentions, data.searchEvents, data.propagationEdges]);
    ...
    <section className="workspace">
      ...
      <div className="content-area">
        {loading ? <LoadingState /> : !data.viewer.authenticated ? <PublicAccess returnTo={routeByView[view]} /> : !data.brand ? <BrandOnboarding submit={async (payload) => { await post(payload, "品牌档案已创建，正在启动全球发现"); void syncNews(true, true); }} /> : <>
          ...
          {view === "archive" && <ArchiveView mentions={filteredMentions} allCount={data.mentions.length} countries={countries} platforms={platforms} platformCounts={platformCounts} country={country} platform={platform} sentiment={sentiment} setCountry={setCountry} setPlatform={setPlatform} setSentiment={setSentiment} submit={post} collectComments={collectMentionComments} canEdit={Boolean(data.workspace?.canEdit)} uiLanguage={language} />}
...
function Overview({ data, brand, clusters, alerts, setView, selectCluster, acknowledge, canEdit, uiLanguage }: { data: DashboardData; brand: BrandProfile; clusters: StoryCluster[]; alerts: Alert[]; setView: (view: ViewId) => void; selectCluster: (key: string) => void; acknowledge: (id: number) => Promise<unknown>; canEdit: boolean; uiLanguage: UiLanguage }) {
  ...
  const mentionCopy = useMentionCopies(data.mentions, uiLanguage);
  return <div className="dashboard-stack">
    ...
    <section className="metric-strip">
      ...
      <Metric label="传播事件" value={String(clusters.length)} note="搜索需求明显攀升时开始识别" />
    ...
    <div className="overview-grid">
      ...
      <section className="surface recent-panel"><div className="section-head"><div><p className="eyebrow">LATEST ARCHIVE</p><h3>最新归档新闻</h3></div><button className="text-button" onClick={() => setView("archive")}>全部档案 →</button></div><div className="latest-list" data-no-ui-translate>{data.mentions.slice(0, 6).map((item) => <article key={item.id}><span className={`tone-dot ${sentimentClass(item.sentiment)}`} /><div><a href={item.url} target="_blank" rel="noreferrer">{mentionCopy(item).title}</a><small>{item.source} · {localizedCountryName(item.source_country, uiLanguage)} · {formatDate(item.published_at)}</small></div><b className={`risk-pill ${riskClass(item.risk)}`}>{item.risk}</b></article>)}</div></section>
      <section className="surface event-panel"><div className="section-head"><div><p className="eyebrow">PROPAGATION EVENTS</p><h3>正在扩散的报道链路</h3></div><span className="count-chip">{clusters.length}</span></div><div className="event-list" data-no-ui-translate>{clusters.slice(0, 5).map((cluster, index) => <button key={cluster.key} onClick={() => selectCluster(cluster.key)}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{mentionCopy(cluster.items[0]).title}</strong><small>{uiLanguage === "en" ? `Starting point: ${localizedCountryName(cluster.originCountry, uiLanguage)} · Coverage in ${cluster.countries.length} regions · ${cluster.items.length} nodes` : `起点：${cluster.originCountry} · 覆盖 ${cluster.countries.length} 个地区 · ${cluster.items.length} 个节点`}</small></div><b>{cluster.items.length}</b><i>→</i></button>)}</div></section>
...
function ArchiveView({ mentions, allCount, countries, platforms, platformCounts, country, platform, sentiment, setCountry, setPlatform, setSentiment, submit, collectComments, canEdit, uiLanguage }: { mentions: Mention[]; allCount: number; countries: string[]; platforms: string[]; platformCounts: Record<string, number>; country: string; platform: string; sentiment: string; setCountry: (value: string) => void; setPlatform: (value: string) => void; setSentiment: (value: string) => void; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; collectComments: (mentionId: number) => Promise<unknown>; canEdit: boolean; uiLanguage: UiLanguage }) {
  ...
  const [commentCollectionId, setCommentCollectionId] = useState(0);
  ...
  const mentionCopy = useMentionCopies(mentions, uiLanguage);
  ...
  async function createManual(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await submit({ action: "createMention", ...values }, "内容已归档，公开互动数据捕捉已启动");
      setManualOpen(false);
  ...
  function interaction(item: Mention) {
    ...
    const captureLabel = ({ queued: "互动数据排队中", running: "正在读取互动数据", completed: "互动数据已更新", metadata_only: "已读取页面资料",
      metadata_complete: "互动数据已更新", failed: "互动数据读取失败" } as Record<string, string>)[item.capture_status ?? ""];
    return <div className="interaction-stack">{metrics.length > 0 && <strong>{metrics.map(([label, value]) => `${label} ${value.toLocaleString()}`).join(" · ")}</strong>}
      {captureLabel && <small title={item.capture_error || undefined}>{captureLabel}{item.capture_updated_at ? ` · ${formatDate(item.capture_updated_at)}` : ""}</small>}
      ...
      {!metrics.length && !Number(item.comment_analyzed_count ?? 0) && !captureLabel && <small>暂无可公开读取的互动数据</small>}
      {canEdit && <button type="button" className="inline-collection-button" disabled={commentCollectionId === item.id} onClick={async () => {
        setCommentCollectionId(item.id); try { await collectComments(item.id); } catch { /* The parent toast explains the failure. */ } finally { setCommentCollectionId(0); }
      }}>{commentCollectionId === item.id ? "正在采集…" : Number(item.comment_analyzed_count ?? 0) > 0 ? "重新采集评论" : "采集并分析评论"}</button>}</div>;
  }
  return <div className="archive-page">
    ...
    <section className="surface archive-table-card">
      ...
      <div className="table-scroll"><table className="archive-table"><thead><tr><th>发布时间</th><th>平台</th><th>新闻 / 社媒原文</th><th>媒体 / 账号</th><th>互动</th><th>地区与语言</th><th>具体情绪</th><th>风险</th><th>事件</th></tr></thead><tbody>{rows.map((item) => { const copy = mentionCopy(item); return <tr key={item.id}><td className="date-cell">{formatDate(item.published_at, true)}</td><td><span className={`platform-badge platform-${item.platform.toLowerCase().replace("网页新闻", "web")}`}>{item.platform}</span></td><td className="title-cell"><a href={item.url} target="_blank" rel="noreferrer">{copy.title}<span>↗</span></a>{item.platform === "Reddit" && <span className="reddit-record-note">{uiLanguage === "en" ? "Keyword discovery · Detail enrichment · On-demand comments" : "关键词发现 · 详情补全 · 评论按需采集"}</span>}<small>{copy.excerpt}</small><EnglishTranslation value={item.translation_en} status={item.translation_status} language={item.language} error={item.translation_error} nextRetryAt={item.translation_next_retry_at} uiLanguage={uiLanguage} /></td><td><strong>{item.source}</strong>{item.author && item.author !== item.source && <small>{item.author}</small>}</td><td>{interaction(item)}</td><td><div className="archive-location"><div><span className="country-tag">{countryCode[item.source_country] ?? "GL"}</span><strong>{item.source_country}</strong></div><small>{item.language} · {item.location_method || "待确认"}{item.location_confidence ? ` · ${item.location_confidence}%` : ""}</small>{canEdit && <button type="button" onClick={() => { setLocationError(""); setLocationMention(item); }}>修改</button>}</div></td><td><span className="emotion-pill">{item.emotion || item.sentiment}</span><small>{item.sentiment}</small></td><td><span className={`risk-score ${riskClass(item.risk)}`}>{item.risk}</span></td><td><code>{item.cluster_key.replace("story-", "#")}</code></td></tr>; })}</tbody></table></div>
...
function PropagationView({ clusters, selected, edges, searchSignals, onSelect, submit, canEdit, uiLanguage }: { clusters: StoryCluster[]; selected?: StoryCluster; edges: PropagationEdge[]; searchSignals: SearchDemandSignal[]; onSelect: (key: string) => void; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canEdit: boolean; uiLanguage: UiLanguage }) {
  const [focusedMentionId, setFocusedMentionId] = useState<number | null>(null);
  const [originOpen, setOriginOpen] = useState(false);
  const [originBusy, setOriginBusy] = useState(false);
  const [originError, setOriginError] = useState("");
  const [networkScale, setNetworkScale] = useState<number | null>(null);
  const [networkViewportWidth, setNetworkViewportWidth] = useState(0);
  const [networkExpanded, setNetworkExpanded] = useState(false);
  const networkViewportRef = useRef<HTMLDivElement>(null);
  const eventMentions = useMemo(() => clusters.flatMap((cluster) => cluster.items), [clusters]);
  const mentionCopy = useMentionCopies(eventMentions, uiLanguage);
  const selectedIds = new Set((selected?.items ?? []).filter((item) => !item.event_origin).map((item) => item.id));
  const selectedEdges = selected ? edges.filter((item) => selectedIds.has(item.from_mention_id) && selectedIds.has(item.to_mention_id)) : [];
  const originNode = selected?.items.find((item) => item.event_origin);
  const firstArchiveNode = selected?.items.find((item) => !item.event_origin);
  if (selected && originNode && firstArchiveNode && !selectedEdges.some((edge) => edge.to_mention_id === firstArchiveNode.id)) {
    selectedEdges.unshift({ id: -Math.abs(originNode.id), cluster_key: selected.key, from_mention_id: originNode.id, to_mention_id: firstArchiveNode.id,
      similarity: 100, confidence: 100, method: "官方首发锚点", evidence: "事件起点由人工确认；后续媒体之间仍按文本相似度与发布时间推断。",
      time_gap_minutes: Math.max(0, Math.round((new Date(firstArchiveNode.published_at).getTime() - new Date(originNode.published_at).getTime()) / 60000)), cross_border: 0 });
  }
  const inferredEdges = selectedEdges.filter((edge) => edge.id > 0);
  const archiveNodes = selected?.items.filter((item) => !item.event_origin) ?? [];
  ...
  const reprintRatio = archiveNodes.length > 1 ? Math.round(inferredEdges.filter((edge) => edge.similarity >= 45).length / Math.max(1, archiveNodes.length - 1) * 100) : 0;
  const avgConfidence = inferredEdges.length ? Math.round(inferredEdges.reduce((sum, edge) => sum + edge.confidence, 0) / inferredEdges.length) : 0;
  ...
  const graphWidth = Math.max(920, (selected?.items.length ?? 1) * 88 + 180);
  const graphHeight = Math.max(280, eventCountries.length * 72 + 70);
  ...
  const nodePositions = new Map((selected?.items ?? []).map((item, index, items) => {
    ...
    return [item.id, {
      x: items.length === 1 ? graphWidth / 2 : 100 + index * ((graphWidth - 200) / (items.length - 1)),
      y: 58 + Math.max(0, eventCountries.findIndex((entry) => entry.region === item.source_country)) * 72 + ((rank % 3) - 1) * 16,
    }];
  }));
  const fitNetworkScale = networkViewportWidth ? Math.min(1, Math.max(.08, (networkViewportWidth - 20) / graphWidth)) : .5;
  const effectiveNetworkScale = networkScale ?? fitNetworkScale;
  ...
  const signalStart = selected ? new Date(`${selected.signal.start_date}T00:00:00Z`).getTime() - 7 * 86400_000 : 0;
  const signalEnd = selected ? new Date(`${selected.signal.end_date}T23:59:59Z`).getTime() + 5 * 86400_000 : 0;
  const signalSeries = searchSignals.filter((item) => { const time = new Date(`${item.signal_date}T12:00:00Z`).getTime(); return time >= signalStart && time <= signalEnd; });
  const maxSearchClicks = Math.max(1, ...signalSeries.map((item) => item.clicks));
  const evidenceKind = selected?.signal.trigger_source ?? "";
  const isSearchEvidence = evidenceKind === "gsc";
  const isSocialEvidence = evidenceKind === "social_spike";
  const mediaVolume = selected ? [...new Map(selected.items.filter((item) => !item.event_origin).map((item) => [item.published_at.slice(0, 10), 0])).keys()]
    .sort().map((date) => ({ date, count: selected.items.filter((item) => !item.event_origin && item.published_at.startsWith(date)).length })) : [];
  const maxMediaVolume = Math.max(1, ...mediaVolume.map((item) => item.count));
  useEffect(() => {
    const viewport = networkViewportRef.current;
    if (!viewport) return;
    const updateWidth = () => setNetworkViewportWidth(viewport.clientWidth);
    updateWidth();
    const observer = new ResizeObserver(updateWidth);
    observer.observe(viewport);
    return () => observer.disconnect();
  }, [selected?.key, networkExpanded]);
  useEffect(() => { setNetworkScale(null); }, [selected?.key, networkExpanded]);
  useEffect(() => {
    if (!networkExpanded) return;
    const previousOverflow = document.body.style.overflow;
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setNetworkExpanded(false); };
    document.body.style.overflow = "hidden";
    document.addEventListener("keydown", closeOnEscape);
    return () => { document.body.style.overflow = previousOverflow; document.removeEventListener("keydown", closeOnEscape); };
  }, [networkExpanded]);
  function changeNetworkZoom(multiplier: number) {
    setNetworkScale(Math.min(2.5, Math.max(.08, effectiveNetworkScale * multiplier)));
  }
  async function createOrigin(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setOriginBusy(true); setOriginError("");
    try { await submit({ action: "createEventOrigin", ...Object.fromEntries(new FormData(event.currentTarget).entries()) }, "事件起点已保存，并与搜索攀升窗口关联"); setOriginOpen(false); }
    catch (error) { setOriginError(error instanceof Error ? error.message : "事件起点保存失败"); }
    finally { setOriginBusy(false); }
  }
  return <div className="propagation-layout">
    <section className="surface cluster-index"><div className="section-head"><div><p className="eyebrow">AUTOMATIC EVENT DETECTION</p><h3>正式事件</h3></div><span className="count-chip">{clusters.length}</span></div><p className="cluster-method-note">系统持续比较搜索需求、媒体发布量与社交互动；明显攀升的第一天建立新事件，随后再用新闻相似度和发布时间判断哪些内容属于该事件。已确认事件只追加，不会被后来的峰值覆盖。</p>{canEdit && <button className="secondary-button event-origin-button" type="button" onClick={() => { setOriginError(""); setOriginOpen(true); }}>＋ 人工校正起点</button>}<div className="cluster-list" data-no-ui-translate>{clusters.map((cluster) => <button key={cluster.key} className={selected?.key === cluster.key ? "selected" : ""} onClick={() => onSelect(cluster.key)}><div><span className={`risk-pill ${riskClass(cluster.risk)}`}>{cluster.signal.status === "active" ? "RISE ACTIVE" : "RISE DETECTED"}</span><small>{cluster.signal.start_date} → peak {cluster.signal.peak_date}</small></div><h4>{cluster.title}</h4><p>{uiLanguage === "en" ? `Starting point: ${cluster.originSource} (${localizedCountryName(cluster.originCountry, uiLanguage)}) · Coverage in ${cluster.countries.length} regions` : `起点：${cluster.originSource}（${cluster.originCountry}） · 覆盖 ${cluster.countries.length} 个地区`}</p><footer><span>{uiLanguage === "en" ? `${cluster.items.length} nodes` : `${cluster.items.length} 节点`}</span><span>{cluster.platforms.map((item) => localizedPlatformName(item, uiLanguage)).join(" · ")}</span><b>→</b></footer></button>)}</div></section>
    <section className="surface propagation-detail">{selected ? <>
      <div className="detail-heading"><div><p className="eyebrow">EVIDENCE-BASED PROPAGATION</p><h2 data-no-ui-translate>{selected.title}</h2><p>Search Console 用于识别搜索需求从哪一天开始明显攀升；事件内容与扩散方向仍由发布时间、标题正文相似度、关键数字、跨语言主题指纹和显式引用共同判断。</p></div><div className="chain-stat"><strong>{selected.countries.length}</strong><span>国家 / 地区</span><small>{selectedEdges.filter((item) => item.cross_border).length} 次跨境传播</small></div></div>
      <section className="search-demand-card"><div className="network-heading"><div><p className="eyebrow">{isSearchEvidence ? "GOOGLE SEARCH DEMAND" : isSocialEvidence ? "SOCIAL ENGAGEMENT SPIKE" : "MEDIA ATTENTION RISE"}</p><h3>事件时间证据</h3><span>攀升开始 {selected.signal.start_date} · 最高点 {selected.signal.peak_date} · 观察至 {selected.signal.end_date}</span></div><div className="search-demand-kpis">{isSearchEvidence ? <><span><b>{selected.signal.peak_clicks ? selected.signal.peak_clicks.toLocaleString() : "待回填"}</b>最高点点击</span><span><b>{selected.signal.peak_impressions ? selected.signal.peak_impressions.toLocaleString() : "待回填"}</b>最高点曝光</span></> : isSocialEvidence ? <><span><b>{selected.signal.peak_clicks.toLocaleString()}</b>公开互动</span><span><b>{selected.signal.peak_impressions.toLocaleString()}</b>播放或浏览</span></> : <><span><b>{selected.signal.peak_clicks.toLocaleString()}</b>峰值内容量</span><span><b>{selected.signal.peak_impressions.toLocaleString()}</b>窗口内容量</span></>}<span><b>{selected.signal.spike_ratio ? `${(selected.signal.spike_ratio / 100).toFixed(1)}×` : "已确认"}</b>相对基线</span></div></div><div className="search-demand-chart">{isSearchEvidence ? signalSeries.map((item) => <div key={item.signal_date} className={`${item.signal_date === selected.signal.peak_date ? "peak" : ""} ${item.signal_date === selected.signal.start_date ? "rise-start" : ""}`} title={`${item.signal_date}：${item.clicks} 次点击，${item.impressions} 次曝光`}><i style={{ height: `${Math.max(5, item.clicks / maxSearchClicks * 100)}%` }} /><small>{item.signal_date.slice(5)}</small></div>) : mediaVolume.map((item) => <div key={item.date} className={`${item.date === selected.signal.peak_date ? "peak" : ""} ${item.date === selected.signal.start_date ? "rise-start" : ""}`} title={`${item.date}：${item.count} 个传播节点`}><i style={{ height: `${Math.max(5, item.count / maxMediaVolume * 100)}%` }} /><small>{item.date.slice(5)}</small></div>)}{isSearchEvidence && !signalSeries.length && <p>系统已保存事件窗口；Search Console 数据同步后会自动补全每日点击、曝光、基线和最高点。</p>}{!isSearchEvidence && !mediaVolume.length && <p>系统已根据高互动首发内容建立事件；后续相关报道进入档案后会自动补全传播曲线。</p>}</div></section>
      <section className={`event-network-card ${networkExpanded ? "expanded" : ""}`}><div className="network-heading"><div><p className="eyebrow">EVENT PROPAGATION GRAPH</p><h3>同一事件扩散路径</h3><span>从左到右按发布时间排列；方块颜色代表发布渠道，点击节点查看证据。</span></div><div className="network-legend"><span><i className="normal" />同地区</span><span><i className="cross" />跨地区</span><span><i className="origin" />最早信源</span>{platformVisuals.map(([label, slug]) => <span key={label}><i className={`channel ${slug}`} />{localizedPlatformName(label, uiLanguage)}</span>)}</div></div><div className="network-toolbar"><span>{uiLanguage === "en" ? `${selected.items.length} items across ${eventCountries.length} regions` : `${selected.items.length} 篇内容 · ${eventCountries.length} 个地区`}</span><div><button type="button" onClick={() => changeNetworkZoom(1 / 1.25)} aria-label={uiLanguage === "en" ? "Zoom out" : "缩小传播图"}>−</button><strong>{Math.round(effectiveNetworkScale * 100)}%</strong><button type="button" onClick={() => changeNetworkZoom(1.25)} aria-label={uiLanguage === "en" ? "Zoom in" : "放大传播图"}>＋</button><button type="button" className={networkScale === null ? "active" : ""} onClick={() => setNetworkScale(null)}>{uiLanguage === "en" ? "Fit" : "适合窗口"}</button><button type="button" onClick={() => setNetworkExpanded((value) => !value)}>{networkExpanded ? (uiLanguage === "en" ? "Exit full screen" : "退出全屏") : (uiLanguage === "en" ? "Full screen" : "全屏查看")}</button></div></div><div className="network-scroll" ref={networkViewportRef}><div className="network-canvas" style={{ width: `${graphWidth * effectiveNetworkScale}px`, height: `${graphHeight * effectiveNetworkScale}px` }}><svg className="event-network" data-no-ui-translate viewBox={`0 0 ${graphWidth} ${graphHeight}`} role="img" aria-label={uiLanguage === "en" ? "Media propagation graph for the selected event" : "同一新闻事件的媒体扩散路径图"}><defs><marker id="arrow-normal" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" /></marker><marker id="arrow-cross" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" /></marker></defs>{eventCountries.map((lane, index) => <g key={lane.region} className="network-lane"><line x1="90" y1={58 + index * 72} x2={graphWidth - 25} y2={58 + index * 72} /><text x="12" y={61 + index * 72}>{localizedCountryName(lane.region, uiLanguage)}</text></g>)}{selectedEdges.map((edge) => { const from = nodePositions.get(edge.from_mention_id); const to = nodePositions.get(edge.to_mention_id); if (!from || !to) return null; const bend = Math.max(28, Math.abs(to.x - from.x) * .32); return <g key={edge.id} className={edge.cross_border ? "network-edge cross" : "network-edge"}><path d={`M ${from.x + 58} ${from.y} C ${from.x + 58 + bend} ${from.y}, ${to.x - 58 - bend} ${to.y}, ${to.x - 58} ${to.y}`} markerEnd={`url(#arrow-${edge.cross_border ? "cross" : "normal"})`} /><text x={(from.x + to.x) / 2} y={(from.y + to.y) / 2 - 6}>{edge.similarity}%</text></g>; })}{selected.items.map((item, index) => { const point = nodePositions.get(item.id)!; const isFocused = focused?.id === item.id; const copy = mentionCopy(item); return <g key={item.id} className={`network-node platform-${platformSlug(item.platform)} ${index === 0 ? "origin" : ""} ${isFocused ? "focused" : ""}`} transform={`translate(${point.x} ${point.y})`} onClick={() => setFocusedMentionId(item.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setFocusedMentionId(item.id); } }} role="button" tabIndex={0}><rect x="-58" y="-24" width="116" height="48" rx="6" /><text className="node-index" x="-49" y="-10">{String(index + 1).padStart(2, "0")} · {localizedPlatformName(item.platform, uiLanguage)}</text><text className="node-source" x="-49" y="5">{item.source.slice(0, 14)}{item.source.length > 14 ? "…" : ""}</text><text className="node-title" x="-49" y="17">{copy.title.slice(0, 18)}{copy.title.length > 18 ? "…" : ""}</text><text className="node-time" x="49" y="-10" textAnchor="end">{formatDate(item.published_at)}</text></g>; })}</svg></div></div>{!hasPropagationSample && <div className="network-empty-note">当前事件只有 1 篇报道，尚不能形成扩散路径；系统会在发现相似后续报道后自动连边。</div>}{focused && <div className="network-evidence"><div><span className="country-tag">{countryCode[focused.source_country] ?? "GL"}</span><div><strong>{focused.source}</strong><small>{localizedCountryName(focused.source_country, uiLanguage)} · {localizedPlatformName(focused.platform, uiLanguage)} · {formatDate(focused.published_at, true)}</small></div></div><div data-no-ui-translate><a href={focused.url} target="_blank" rel="noreferrer">{mentionCopy(focused).title} ↗</a><p>{mentionCopy(focused).excerpt}</p></div><aside>{focusedEdge ? <><b>{focusedEdge.method}</b><span>{focusedEdge.confidence}% 置信度 · {focusedEdge.similarity}% 内容重合</span><small>推断上游：{focusedParent?.source ?? "公开信源"} · 间隔 {gapLabel(focusedEdge.time_gap_minutes)}<br />{focusedEdge.evidence}</small></> : <><b>事件起点</b><span>当前聚类中的最早公开报道</span><small>后续报道将通过箭头连接到最可能的上游来源。</small></>}</aside></div>}</section>
      <div className="event-analysis-strip"><div><span>事件窗口</span><strong>{selected.signal.start_date.slice(5)}—{selected.signal.end_date.slice(5)}</strong><small>从明显攀升首日开始</small></div><div><span>传播节点峰值</span><strong>{hasPropagationSample ? formatDate(new Date(eventStart + peak[0] * 3600_000).toISOString()) : "样本不足"}</strong><small>{hasPropagationSample ? `该小时新增 ${peak[1]} 个节点` : "至少需要 2 个传播节点"}</small></div><div><span>高度转载率</span><strong>{inferredEdges.length ? `${reprintRatio}%` : "样本不足"}</strong><small>内容重合度 ≥ 45%</small></div><div><span>平均推断置信</span><strong>{avgConfidence ? `${avgConfidence}%` : "样本不足"}</strong><small>{avgConfidence ? "新闻相似度证据评分" : "尚未形成传播边"}</small></div><div><span>传播跨度</span><strong>{hasPropagationSample ? eventSpanHours < 1 ? "<1h" : eventSpanHours < 24 ? `${eventSpanHours}h` : `${Math.round(eventSpanHours / 24)}d` : "样本不足"}</strong><small>首发起点至最后节点</small></div></div>
      <div className="event-visuals"><section><div className="mini-title"><div><p className="eyebrow">PROPAGATION BURST</p><h3>传播节点爆发曲线</h3></div><span>从首发起点起</span></div><div className="burst-chart">{burstSeries.length ? burstSeries.map(([hour, count]) => <div key={hour} title={`首发后 ${hour} 小时：${count} 个节点`}><i style={{ height: `${Math.max(8, count / maxBurst * 100)}%` }} /><span>+{hour}h</span></div>) : <small>等待更多报道</small>}</div></section><section><div className="mini-title"><div><p className="eyebrow">REGION × SOURCE</p><h3>地区与信源构成</h3></div></div><div className="composition-list">{eventCountries.map((item) => <div key={item.region}><span>{item.region}</span><i><b style={{ width: `${item.count / Math.max(1, selected.items.length) * 100}%` }} /></i><strong>{item.count}</strong></div>)}</div><div className="source-chips">{eventSources.map((item) => <span key={item.source}>{item.source}<b>{item.count}</b></span>)}</div></section></div>
      {comparableClusters.length > 1 && <div className="event-comparison"><div className="section-head"><div><p className="eyebrow">CROSS-EVENT COMPARISON</p><h3>并列事件对比</h3></div><span className="subtle-note">仅比较已形成传播链的事件</span></div><div className="comparison-layout"><div className="event-scatter"><span className="axis-label y">高风险 ↑</span><span className="axis-label x">传播速度 →</span>{comparableClusters.slice(0, 12).map((cluster, index) => { const start = new Date(cluster.items[0].published_at).getTime(); const end = new Date(cluster.items.at(-1)!.published_at).getTime(); const hours = Math.max(1, (end - start) / 3600_000); const velocity = cluster.items.length / hours; const maxVelocity = Math.max(.01, ...comparableClusters.slice(0, 12).map((item) => { const first = new Date(item.items[0].published_at).getTime(); const last = new Date(item.items.at(-1)!.published_at).getTime(); return item.items.length / Math.max(1, (last - first) / 3600_000); })); return <button key={cluster.key} className={cluster.key === selected.key ? "active" : ""} style={{ left: `${12 + velocity / maxVelocity * 76}%`, bottom: `${10 + cluster.risk / 100 * 75}%`, width: `${18 + Math.min(34, cluster.items.length * 4)}px`, height: `${18 + Math.min(34, cluster.items.length * 4)}px` }} title={`${mentionCopy(cluster.items[0]).title} · ${cluster.items.length} items · Risk ${cluster.risk}`} onClick={() => onSelect(cluster.key)}>{index + 1}</button>; })}</div><div className="comparison-table" data-no-ui-translate>{comparableClusters.slice(0, 6).map((cluster, index) => <button key={cluster.key} onClick={() => onSelect(cluster.key)}><span>{index + 1}</span><div><strong>{mentionCopy(cluster.items[0]).title}</strong><small>{uiLanguage === "en" ? `${cluster.countries.length} regions · ${cluster.items.length} items` : `${cluster.countries.length} 地区 · ${cluster.items.length} 篇`}</small></div><b className={`risk-score ${riskClass(cluster.risk)}`}>{cluster.risk}</b></button>)}</div></div></div>}
    </> : <div className="empty-state">当前没有检测到搜索需求明显攀升的传播事件。连接 Search Console，或先人工登记已知的攀升开始日期与首发起点。</div>}</section>
    {originOpen && <div className="credential-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setOriginOpen(false); }}><form className="credential-modal event-origin-modal" onSubmit={createOrigin}><div className="section-head"><div><p className="eyebrow">EVENT ORIGIN</p><h3>登记攀升开始与首发起点</h3></div><button type="button" className="modal-close" onClick={() => setOriginOpen(false)}>×</button></div><p>这里登记搜索需求开始明显攀升的日期与可核验的首发内容。官方账号帖子只进入传播图，不会重新出现在外部新闻档案中。</p><label className="field"><span>明显攀升开始日期 *</span><input name="eventDate" type="date" required defaultValue={selected?.signal.start_date ?? new Date().toISOString().slice(0, 10)} /></label><label className="field"><span>首发发布时间 *</span><input name="publishedAt" type="datetime-local" required /></label><label className="field"><span>首发平台</span><select name="platform" defaultValue="X"><option>X</option><option>Instagram</option><option>TikTok</option><option>YouTube</option><option>Facebook</option><option>Reddit</option><option>网页新闻</option></select></label><label className="field"><span>账号 / 来源 *</span><input name="source" required placeholder="例如：@brand_official" /></label><label className="field full"><span>事件标题 *</span><input name="title" required placeholder="用一句话说明这次攀升由什么内容触发" /></label><label className="field full"><span>首发原文链接 *</span><input name="url" type="url" required placeholder="https://…" /></label><label className="field"><span>来源国家 / 地区</span><input name="sourceCountry" defaultValue="全球" /></label><label className="field full"><span>起点说明</span><textarea name="note" rows={3} placeholder="说明为何将它确认为传播起点，以及可核验的依据" /></label>{originError && <p className="form-error">{originError}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setOriginOpen(false)}>取消</button><button className="primary-button" disabled={originBusy}>{originBusy ? "保存中…" : "保存事件起点"}</button></div></form></div>}
  </div>;
}

function AnalyticsView({ analytics, mentions, uiLanguage }: { analytics: Analytics; mentions: Mention[]; uiLanguage: UiLanguage }) {
  ...
  const contentTermTranslation = useAmericanEnglishBatch(analytics.words.map((item) => item.word), uiLanguage);
  const commentTermTranslation = useAmericanEnglishBatch(visibleCommentWords.map((item) => item.word), uiLanguage);
  const localizedWords = analytics.words.flatMap((item) => {
    const word = contentTermTranslation.values[item.word];
    return word ? [{ ...item, word }] : [];
  });
  const localizedCommentWords = visibleCommentWords.flatMap((item) => {
    const word = commentTermTranslation.values[item.word];
    return word ? [{ ...item, word }] : [];
  });
  ...
  return <div className="analysis-grid">
    ...
    <section className="surface wordcloud-card"><div className="section-head"><div><p className="eyebrow">KEYWORD CLOUD</p><h3>高频议题词云</h3></div><span className="subtle-note">已排除品牌名与常见停用词</span></div><div className="word-cloud" data-no-ui-translate>{localizedWords.map((item, index) => <span key={`${item.word}-${item.count}`} className={index < 6 ? "hot" : ""} style={{ fontSize: `${11 + item.count / maxWord * 25}px`, opacity: .5 + item.count / maxWord * .5 }} title={uiLanguage === "en" ? `${item.count} mentions` : `${item.count} 次`}>{item.word}<sup>{item.count}</sup></span>)}{contentTermTranslation.loading && <small className="word-cloud-translation-state">Translating analysis terms…</small>}{uiLanguage === "en" && !contentTermTranslation.loading && !localizedWords.length && <small className="word-cloud-translation-state">No translated terms are available.</small>}</div></section>
    ...
    <section className="surface wordcloud-card comment-wordcloud-card"><div className="section-head"><div><p className="eyebrow">COMMENT KEYWORD CLOUD</p><h3>评论区关键词词云</h3></div><span className="subtle-note">仅基于实际取得的公开评论</span></div>{visibleCommentWords.length ? <div className="word-cloud" data-no-ui-translate>{localizedCommentWords.map((item, index) => <span key={`${item.word}-${item.count}`} className={index < 6 ? "hot" : ""} style={{ fontSize: `${11 + item.count / maxCommentWord * 25}px`, opacity: .5 + item.count / maxCommentWord * .5 }} title={uiLanguage === "en" ? `${item.count} mentions` : `${item.count} 次`}>{item.word}<sup>{item.count}</sup></span>)}{commentTermTranslation.loading && <small className="word-cloud-translation-state">Translating analysis terms…</small>}</div> : <div className="comment-empty">暂无可用于词频统计的公开评论文本</div>}</section>
...
function AudienceWordCloud({ words, uiLanguage, maxWord }: { words: Array<{ word: string; count: number }>; uiLanguage: UiLanguage; maxWord: number }) {
  const translation = useAmericanEnglishBatch(words.map((item) => item.word), uiLanguage);
  const localized = words.flatMap((item) => translation.values[item.word] ? [{ ...item, word: translation.values[item.word] }] : []);
  return <section className="surface comment-cloud-card"><div className="section-head"><div><p className="eyebrow">MEANINGFUL TERMS</p><h3>评论高频词云</h3></div><span className="subtle-note">中英文分词 · 已过滤虚词</span></div>{words.length ? <div className="word-cloud" data-no-ui-translate>{localized.map((item, index) => <span key={`${item.word}-${item.count}`} className={index < 6 ? "hot" : ""} style={{ fontSize: `${11 + item.count / maxWord * 25}px`, opacity: .5 + item.count / maxWord * .5 }} title={uiLanguage === "en" ? `${item.count} mentions` : `${item.count} 次`}>{item.word}<sup>{item.count}</sup></span>)}{translation.loading && <small className="word-cloud-translation-state">Translating analysis terms…</small>}</div> : <div className="comment-empty">暂无可统计的有效词</div>}</section>;
}
...
function SocialCommentsView({ brand, monidConfigured, canEdit, uiLanguage }: { brand: BrandProfile; monidConfigured: boolean; canEdit: boolean; uiLanguage: UiLanguage }) {
  ...
  const visibleCommentRows = useMemo(() => [...(data?.comments ?? []), ...(data?.riskComments ?? [])], [data?.comments, data?.riskComments]);
  const commentTranslations = useAmericanEnglishBatch(visibleCommentRows.filter((item) => !sourceIsEnglish(item.language) && !item.translation_en?.trim()).map((item) => item.content), uiLanguage);
  const postTitleRecords = useMemo(() => [
    ...(data?.topPosts ?? []).map((item) => ({ title: item.title, translation: item.translation_en, language: item.language })),
    ...(data?.targets ?? []).map((item) => ({ title: item.post_title, translation: item.post_translation_en, language: item.post_language })),
    ...visibleCommentRows.map((item) => ({ title: item.post_title, translation: item.post_translation_en, language: item.post_language })),
  ], [data?.targets, data?.topPosts, visibleCommentRows]);
  const postTitleTranslations = useAmericanEnglishBatch(postTitleRecords.filter((item) => !sourceIsEnglish(item.language) && !item.translation?.trim()).map((item) => item.title), uiLanguage);
  const commentCopy = (item: SocialCommentRow) => translatedCommentCopy({ ...item, translation_en: item.translation_en?.trim() || commentTranslations.values[item.content] || "" }, uiLanguage);
  const postTitleCopy = (title: string, translation?: string, language?: string, fallback?: string) => translatedStoredTitle(title, translation?.trim() || postTitleTranslations.values[title] || "", language, uiLanguage, fallback);
  ...
  async function collectDirectPost(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDirectMessage(`${result.platform} 评论采集已由你手动开启，正在识别帖子并分页获取公开评论`);
      setDirectPostUrl("");
  ...
  return <div className="comments-page">
    <section className="comment-hero panel-dark">
      ...
      <div className="comment-collection-state"><span className={monidConfigured ? "online" : "offline"} /><div><small>ON-DEMAND COMMENT COLLECTION</small><strong>{!monidConfigured ? "尚未配置" : !data?.targets.length ? "等待手动选择帖子" : data.targets.some((item) => ["running", "queued", "retrying", "collecting"].includes(item.status)) ? "手动任务采集中" : data.targets.some((item) => ["blocked", "unavailable", "empty", "not_returned", "error", "review"].includes(item.status)) ? "部分帖子待核验" : "当前手动队列已完成"}</strong><em>{summary.collected.toLocaleString()} / {summary.reported.toLocaleString()} 条已归档 · {summary.coverage}%</em></div></div>
    </section>

    <section className="surface comment-pipeline" aria-label="手动开启的评论舆情处理流程"><div><b>01</b><strong>自动归档帖子</strong><span>只保留内容与互动数</span></div><i>→</i><div><b>02</b><strong>人工选择目标</strong><span>点击采集并分析评论</span></div><i>→</i><div><b>03</b><strong>按需采集评论</strong><span>主评论与回复分页</span></div><i>→</i><div><b>04</b><strong>内部语义分析</strong><span>分词、情绪与议题</span></div><i>→</i><div><b>05</b><strong>舆情展示</strong><span>词云、趋势与风险</span></div></section>
    ...
    {!monidConfigured && <section className="comment-callout surface"><strong>社媒评论采集尚未配置</strong><p>评论正文不会自动采集。请先在“数据采集”配置 Monid，再对需要分析的帖子手动点击采集。</p></section>}
    ...
    <div className="comment-intelligence-grid">
      ...
      <AudienceWordCloud words={data?.words ?? []} uiLanguage={uiLanguage} maxWord={maxWord} />
    </div>
    ...
    <section className="surface risk-queue"><div className="section-head"><div><p className="eyebrow">RISK REVIEW QUEUE</p><h3>负面与混合情绪复核</h3></div><span className="subtle-note">按情绪分与互动量排序</span></div><div>{data?.riskComments.map((comment) => <article key={comment.id}><header><span className={`sentiment-pill ${sentimentClass(comment.sentiment)}`}>{comment.sentiment}</span><strong>{comment.likes} 赞</strong></header><p>{commentCopy(comment)}</p><EnglishTranslation value={comment.translation_en} status={comment.translation_status} language={comment.language} error={comment.translation_error} nextRetryAt={comment.translation_next_retry_at} uiLanguage={uiLanguage} /><a href={comment.post_url} target="_blank" rel="noreferrer">{postTitleCopy(comment.post_title, comment.post_translation_en, comment.post_language, `${comment.platform} post`)} ↗</a></article>)}{!data?.riskComments.length && <div className="comment-empty compact">暂无需要复核的高风险评论</div>}</div></section>

    <section className="surface comment-progress-card"><div className="section-head"><div><p className="eyebrow">COLLECTION STATUS</p><h3>手动评论任务状态</h3></div><span className="count-chip">{data?.targets.length ?? 0} 个帖子</span></div><div className="comment-progress-list">{data?.targets.map((target) => { const pct = target.reported_count ? Math.min(100, Math.round(target.collected_count / target.reported_count * 100)) : target.status === "complete" ? 100 : 0; return <article key={target.mention_id}><div><a href={target.mention_url} target="_blank" rel="noreferrer">{postTitleCopy(target.post_title, target.post_translation_en, target.post_language, `${target.post_source || target.platform} post`)}</a><small>{target.platform} · {target.post_source} · {String(target.adapter || "v2").toUpperCase()} · 已请求 {target.pages_fetched} 页 · 失败 {target.failure_count ?? (target.v2_failures || 0) + (target.v1_failures || 0)} 次</small>{target.last_error && <p>{target.last_error}</p>}{target.next_retry_at && target.status === "retrying" && <em>下次重试：{formatDate(target.next_retry_at, true)}</em>}{target.status === "review" && <em>已停止自动重试，请人工核验帖子地址、公开状态和接口返回。</em>}</div><span><i><b style={{ width: `${pct}%` }} /></i><em>{target.collected_count} / {target.reported_count || "?"}</em></span><strong className={target.status}>{targetStatus(target.status)}</strong></article>; })}{!data?.targets.length && <div className="comment-empty compact">尚未手动开启评论任务。可在新闻档案选择具体内容，或在上方粘贴帖子链接。</div>}</div></section>
  </div>;
}
...
function ProductGuide() {
  return <article className="product-guide">
    ...
    <div className="guide-layout">
      ...
      <section className="surface" id="guide-archive"><span>03</span><h3>采集、归档与数据缺失</h3><p>内容档案自动保留发布时间、平台、原文、媒体或账号、地区、原始链接、事件编号和公开互动数据。手动补充链接后也会立即尝试读取点赞、评论数、分享、播放或浏览数据。平台没有披露某项指标时字段留空，只有接口明确返回零时才显示零。</p><p>评论正文和评论情绪分析采用按需模式，不再随日常巡检自动采集。用户需要在新闻档案或受众舆情页面选择具体帖子，点击“采集并分析评论”后，系统才按“采集主评论、采集回复、翻译与分析”的顺序调用接口；历史评论和人工标注会继续保留。</p><p>系统会把品牌名、无空格写法、别名、产品、事件指纹、官网域名以及“品牌名＋身份锚点”组合成搜索计划。Instagram 使用适合 hashtag 的无空格词形，YouTube 与 TikTok 会分别执行前三组高优先级关键词；NewsAPI.ai 在首批达到一百条时自动继续请求后续页面。搜索与归档不会自动触发评论正文采集。</p><p>系统通过URL、平台内容ID和正文相似度识别重复内容。重复内容只保留一条主记录，但不同媒体之间的转载关系仍可进入传播链路。人工补充会保留标记，并与自动采集内容一起参与后续分析。“数据采集”页会分别显示接口候选、相关候选、新增、重复、过滤和失败原因，以便判断漏收发生在哪一层。</p><p>添加排除词后，命中内容会从新闻档案、事件、内容舆情、受众舆情、词云和报告中移除。原始记录可以保留用于审计，但不再参与正常指标。删除排除词后，系统可以重新评估此前被过滤的数据。</p></section>
      <section className="surface" id="guide-events"><span>04</span><h3>同一事件与传播链路</h3><p>事件时间由搜索需求、媒体发布量和社交互动共同确定。Google Search Console 的搜索需求明显脱离近期基线，某日媒体发布量快速增加，或一条相关社交帖子获得显著高于常态的互动时，系统会从攀升开始日自动建立事件。后来的最高点只记录关注度何时达到最大，不会替换较早事件；两个峰值之间出现明显低谷时，分别保存为两个事件。</p><p>系统随后在每个事件窗口内继续使用新闻相似度方法，比较发布时间、标题、正文、人物、产品、地点、关键事实、相同段落和引用来源，只把内容高度相关的报道与帖子纳入对应传播图。传播起点优先采用能够核验的高互动首发帖子、官方原帖、明确引用来源或最早公开报道。直接引用、链接或明显文本复制属于高置信关系；时间明确且内容高度相似但没有直接引用时属于中置信；只有时间和话题接近时属于低置信。自动判断结果可以人工校正，但新事件不会覆盖已经确认的历史事件。</p></section>
...
function CoverageView({ connectors, sources, diagnostics, submit, canManage, uiLanguage }: { connectors: Connector[]; sources: MediaSource[]; diagnostics: CollectionDiagnostic[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canManage: boolean; uiLanguage: UiLanguage }) {
  ...
  async function saveCredential(event: FormEvent<HTMLFormElement>) {
    ...
    const value = editing === "Meta / Instagram" ? JSON.stringify({ accessToken: credential.trim(), accountId: accountId.trim() })
      ...
      : editing === "Bright Data" ? JSON.stringify({ token: credential.trim(), zone: secondaryCredential.trim() })
      : editing === "Mastodon" ? JSON.stringify({ instance: credential.trim(), token: secondaryCredential.trim() })
      : editing === "Google Search Console" ? JSON.stringify({ credentials: credential.trim(), property: secondaryCredential.trim() })
    ...
    const lastFour = editing === "TikTok" || editing === "Mastodon" ? secondaryCredential.slice(-4)
      : editing === "Google Search Console" ? "GSC"
  ...
  return <div className="coverage-page">
    ...
    {editing && canManage && <div className="credential-modal-backdrop" onMouseDown={() => setEditing(null)}><form className="credential-modal" onSubmit={saveCredential} onMouseDown={(event) => event.stopPropagation()}>
      ...
      {editing === "Tumblr Tagged" && <p>使用 Tumblr 免费开发者应用的 OAuth Consumer Key，按品牌、产品和事件标签检索公开多语言博客。接口只返回带有对应标签的帖子，不会搜索私人博客。</p>}
      {editing === "Tumblr Tagged" && <a className="credential-help-link" href="https://www.tumblr.com/oauth/apps" target="_blank" rel="noreferrer">前往 Tumblr 注册应用并获取 Consumer Key ↗</a>}
      ...
      {editing === "Media Cloud" && <a className="credential-help-link" href="https://search.mediacloud.org/" target="_blank" rel="noreferrer">前往 Media Cloud 注册并获取 API Key ↗</a>}
      {editing === "NewsData.io" && <a className="credential-help-link" href="https://newsdata.io/register" target="_blank" rel="noreferrer">前往 NewsData.io 创建免费 API Key ↗</a>}
      {editing === "World News API" && <a className="credential-help-link" href="https://worldnewsapi.com/register" target="_blank" rel="noreferrer">前往 World News API 创建免费 API Key ↗</a>}
      {editing === "ScrapeCreators" && <a className="credential-help-link" href="https://app.scrapecreators.com/" target="_blank" rel="noreferrer">前往 ScrapeCreators 创建 API Key ↗</a>}
      {editing === "Brave Search" && <a className="credential-help-link" href="https://api-dashboard.search.brave.com/app/keys" target="_blank" rel="noreferrer">前往 Brave Search 创建 API Key ↗</a>}
      {editing === "Apify" && <a className="credential-help-link" href="https://console.apify.com/account/integrations" target="_blank" rel="noreferrer">前往 Apify 获取 API Token ↗</a>}
      {editing === "Bright Data" && <a className="credential-help-link" href="https://brightdata.com/cp/zones" target="_blank" rel="noreferrer">前往 Bright Data 创建 SERP Zone ↗</a>}
      {editing === "The News API" && <a className="credential-help-link" href="https://www.thenewsapi.com/register" target="_blank" rel="noreferrer">前往 The News API 注册并获取 Token ↗</a>}
      {editing === "GNews" && <a className="credential-help-link" href="https://gnews.io/register" target="_blank" rel="noreferrer">前往 GNews 注册并选择许可方案 ↗</a>}
      {editing === "NewsAPI.org" && <a className="credential-help-link" href="https://newsapi.org/register" target="_blank" rel="noreferrer">前往 NewsAPI.org 注册并选择许可方案 ↗</a>}
      {editing === "mediastack" && <a className="credential-help-link" href="https://mediastack.com/signup/free" target="_blank" rel="noreferrer">前往 mediastack 注册并获取 Access Key ↗</a>}
      {editing === "Guardian Open Platform" && <a className="credential-help-link" href="https://open-platform.theguardian.com/access/" target="_blank" rel="noreferrer">前往 Guardian 申请开发者或商业密钥 ↗</a>}
      {editing === "Mastodon" && <a className="credential-help-link" href="https://docs.joinmastodon.org/client/token/" target="_blank" rel="noreferrer">查看 Mastodon 只读访问令牌说明 ↗</a>}
      {editing === "Google Search Console" && <a className="credential-help-link" href="https://developers.google.com/webmaster-tools/v1/how-tos/authorizing" target="_blank" rel="noreferrer">查看 Search Console 授权说明 ↗</a>}
      ...
      {editing === "Google Search Console" ? <><label className="field"><span>Search Console 资源 *</span><input autoComplete="off" autoFocus required value={secondaryCredential} onChange={(event) => setSecondaryCredential(event.target.value)} placeholder="sc-domain:example.com" /></label><label className="field"><span>服务账号或 OAuth JSON *</span><textarea rows={9} autoComplete="off" required value={credential} onChange={(event) => setCredential(event.target.value)} placeholder={'{\n  "type": "service_account",\n  "client_email": "…",\n  "private_key": "…"\n}'} /></label></> : <label className="field"><span>{editing === "Mastodon" ? "Mastodon 实例地址" : editing === "Monid / Instagram" ? "Monid API Key" : editing === "OpenAI LLM" ? "OpenAI API Key" : editing === "Meta / Instagram" ? "Long-lived Access Token" : editing === "TikTok" ? "Client Key" : editing === "Apify" ? "API Token" : editing === "Azure Translator" ? "Azure Translator Key" : editing === "LibreTranslate" ? "LibreTranslate HTTPS 地址" : editing === "MyMemory" ? "联系邮箱" : editing === "X" ? "Bearer Token" : "API Key"}</span><input type={["LibreTranslate", "Mastodon"].includes(editing) ? "url" : editing === "MyMemory" ? "email" : "password"} autoComplete="off" autoFocus={editing !== "Meta / Instagram"} required value={credential} onChange={(event) => setCredential(event.target.value)} placeholder={editing === "Mastodon" ? "https://mastodon.social" : editing === "Monid / Instagram" ? "monid_live_…" : editing === "OpenAI LLM" ? "sk-…" : editing === "LibreTranslate" ? "https://translate.example.com" : editing === "MyMemory" ? "name@company.com" : "粘贴凭证"} /></label>}
      ...
      {editing === "Bright Data" && <label className="field"><span>SERP Zone 名称</span><input autoComplete="off" required value={secondaryCredential} onChange={(event) => setSecondaryCredential(event.target.value)} placeholder="例如：serp_api1" /></label>}
      {editing === "Mastodon" && <label className="field"><span>只读 Access Token</span><input type="password" autoComplete="off" required value={secondaryCredential} onChange={(event) => setSecondaryCredential(event.target.value)} placeholder="粘贴 read:search 令牌" /></label>}
      <div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setEditing(null)}>取消</button><button className="primary-button" disabled={busy || !credential.trim() || (editing === "Meta / Instagram" && !accountId.trim()) || (["TikTok", "Bright Data", "Google Search Console", "Mastodon"].includes(editing) && !secondaryCredential.trim())}>{busy ? "保存中…" : "安全保存"}</button></div>
    </form></div>}
