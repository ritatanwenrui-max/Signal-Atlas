...
function PropagationView({ clusters, selected, edges, searchSignals, onSelect, submit, canEdit, uiLanguage }: { clusters: StoryCluster[]; selected?: StoryCluster; edges: PropagationEdge[]; searchSignals: SearchDemandSignal[]; onSelect: (key: string) => void; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canEdit: boolean; uiLanguage: UiLanguage }) {
  ...
  return <div className="propagation-layout">
    ...
    <section className="surface propagation-detail">{selected ? <>
      ...
      <section className="search-demand-card"><div className="network-heading"><div><p className="eyebrow">GOOGLE SEARCH DEMAND</p><h3>事件时间证据</h3><span>攀升开始 {selected.signal.start_date} · 最高点 {selected.signal.peak_date} · 观察至 {selected.signal.end_date}</span></div><div className="search-demand-kpis"><span><b>{selected.signal.peak_clicks ? selected.signal.peak_clicks.toLocaleString() : "待回填"}</b>最高点点击</span><span><b>{selected.signal.peak_impressions ? selected.signal.peak_impressions.toLocaleString() : "待回填"}</b>最高点曝光</span><span><b>{selected.signal.spike_ratio ? `${(selected.signal.spike_ratio / 100).toFixed(1)}×` : "已确认"}</b>相对基线</span></div></div><div className="search-demand-chart">{signalSeries.map((item) => <div key={item.signal_date} className={`${item.signal_date === selected.signal.peak_date ? "peak" : ""} ${item.signal_date === selected.signal.start_date ? "rise-start" : ""}`} title={`${item.signal_date}：${item.clicks} 次点击，${item.impressions} 次曝光`}><i style={{ height: `${Math.max(5, item.clicks / maxSearchClicks * 100)}%` }} /><small>{item.signal_date.slice(5)}</small></div>)}{!signalSeries.length && <p>攀升开始日期已人工确认；连接 Google Search Console 后自动回填每日点击、曝光、基线与最高点。</p>}</div></section>
...
function ProductGuide() {
  return <article className="product-guide">
    ...
    <div className="guide-layout">
      ...
      <section className="surface" id="guide-events"><span>04</span><h3>同一事件与传播链路</h3><p>事件时间由两层证据共同确定。Google Search Console 的搜索需求一旦明显脱离近期基线并持续攀升，第一天就视为事件开始；后来的最高点只记录这次事件何时达到最大关注度，不会被误当成起点。系统随后在这个时间窗口内继续使用原有的新闻相似度方法，比较标题、正文、人物、产品、地点、关键事实、相同段落和引用来源，只把内容高度相关的报道与帖子纳入同一传播图。</p><p>传播起点优先采用能够核验的官方原帖、明确引用来源或最早公开报道。传播链路只能从较早发布的内容指向较晚发布的内容。直接引用、链接或明显文本复制属于高置信关系；时间明确且内容高度相似但没有直接引用时属于中置信；只有时间和话题接近时属于低置信。搜索曲线负责回答“事件何时开始受到关注”，文本相似度负责回答“哪些内容属于该事件以及如何传播”，两者不会互相替代。</p></section>
