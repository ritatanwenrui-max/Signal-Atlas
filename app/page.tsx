...
function CoverageView({ connectors, sources, diagnostics, submit, canManage }: { connectors: Connector[]; sources: MediaSource[]; diagnostics: CollectionDiagnostic[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canManage: boolean }) {
  ...
  return <div className="coverage-page">
    <section className="coverage-architecture panel-dark"><div><h2>采集计划与混合分析</h2><p>新闻档案优先使用多个独立新闻源发现新内容；搜索与归档确认没有新增后，后台才进入帖子评论、回复和受众分析阶段。</p></div><div className="architecture-flow"><article><b>01</b><strong>全球新闻发现</strong><span>NewsAPI.ai / Media Cloud / NewsData / World News / GDELT</span><small>按各自额度分时运行</small></article><i>→</i><article><b>02</b><strong>免费来源跟踪</strong><span>RSS / Atom / News Sitemap</span><small>活跃 3 小时 / 探测 12 小时</small></article><i>→</i><article><b>03</b><strong>规则全量分析</strong><span>地区 / 事件 / 情绪 / 传播</span><small>人工标注优先</small></article><i>→</i><article><b>04</b><strong>受众采集</strong><span>评论 / 回复 / 翻译 / 语义</span><small>新闻阶段完成后运行</small></article></div></section>
    <section className="surface collection-diagnostics"><div className="section-head"><div><p className="eyebrow">DAILY API BUDGET</p><h3>每日新闻更新与额度计划</h3></div><span className="diagnostic-state complete">UTC 自然日重置</span></div>
    ...
    {editing && canManage && <div className="credential-modal-backdrop" onMouseDown={() => setEditing(null)}><form className="credential-modal" onSubmit={saveCredential} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <p>{editing === "Media Cloud" ? "用于检索 Media Cloud 全球在线新闻档案。系统每 4 小时运行一次，按每日安全预算计数。" : editing === "NewsData.io" ? "用于补充全球多语言最新新闻。系统每 2 小时运行一次，并为分页、重试和人工补扫保留免费 credits。" : editing === "World News API" ? "用于补充来源国家、语言、摘要、正文和情绪字段。系统每 6 小时运行一次，并按 points 安全预扣。" : editing === "Monid / Instagram" ? "一个密钥启用 Instagram、X、YouTube、TikTok、Facebook、Reddit 的普通文字关键词搜索，以及各平台可公开取得的帖子评论与回复采集。保存时会先验证密钥。" : editing === "OpenAI LLM" ? "用于混合分析中的重点语义复核和报告 Agent。规则模型仍处理全部数据；LLM 只分析高风险、高互动、跨语言或判断不明确的内容。人工标注始终具有最高优先级。" : editing === "Meta / Instagram" ? "用于采集 Business / Creator 账号的标签与 @提及；启用仍取决于 Meta 权限和 App Review。" : editing === "TikTok" ? "用于 TikTok Research API 的公开内容查询；启用仍取决于 Research API 审批。" : editing === "Azure Translator" ? "推荐使用 Azure Translator F0 免费层作为后台自动翻译主力。填写资源密钥；区域按 Azure 资源页面显示填写，单服务全局资源可留空。" : editing === "DeepL API Free" ? "DeepL API Free 每月提供免费字符额度，系统会把它作为 Azure 或自托管翻译失败时的备用。" : editing === "LibreTranslate" ? "填写你自己部署的 LibreTranslate HTTPS 地址；若实例没有启用 API Key，第二项可留空。" : editing === "MyMemory" ? "填写联系邮箱可使用 MyMemory 的已识别免费配额。它仍只作为最后一级兜底，不建议单独承担全部翻译。" : `输入你自己的 ${editing === "X" ? "Bearer Token" : "API Key"}。`} 保存后仅服务端可以解密使用。</p>
