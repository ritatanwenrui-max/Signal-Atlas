...
function PropagationView({ clusters, selected, edges, onSelect }: { clusters: StoryCluster[]; selected?: StoryCluster; edges: PropagationEdge[]; onSelect: (key: string) => void }) {
  ...
  const graphWidth = Math.max(1080, (selected?.items.length ?? 1) * 235);
  const graphHeight = Math.max(300, eventCountries.length * 100 + 80);
  const nodePositions = new Map((selected?.items ?? []).map((item, index, items) => [item.id, {
    x: items.length === 1 ? graphWidth / 2 : 150 + index * ((graphWidth - 300) / (items.length - 1)),
    y: 80 + Math.max(0, eventCountries.findIndex((entry) => entry.region === item.source_country)) * 100,
  }]));
