...
type Mention = {
  ...
  archived_at: string; published_at: string;
};
...
const countryCode: Record<string, string> = {
  ...
  印度尼西亚: "ID", 菲律宾: "PH", 越南: "VN", 马来西亚: "MY", 地区未披露: "--",
};
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="system-card">
        ...
        <div className="system-row"><span>全球发现</span><strong>每 6 小时</strong></div>
        <div className="system-row"><span>免费源追踪</span><strong>每小时</strong></div>
...
function Overview({ data, brand, clusters, alerts, setView, selectCluster, acknowledge }: { data: DashboardData; brand: BrandProfile; clusters: StoryCluster[]; alerts: Alert[]; setView: (view: string) => void; selectCluster: (key: string) => void; acknowledge: (id: number) => Promise<unknown> }) {
  ...
  return <div className="dashboard-stack">
    ...
    <section className="metric-strip">
      ...
      <Metric label="传播事件" value={String(clusters.length)} note="按主题与文本相似度聚类" />
      <Metric label="国家 / 地区" value={String(data.analytics.countries.filter((item) => item.country !== "地区未披露").length)} note="由媒体归属地自动判断" />
...
function PropagationView({ clusters, selected, edges, onSelect }: { clusters: StoryCluster[]; selected?: StoryCluster; edges: PropagationEdge[]; onSelect: (key: string) => void }) {
  ...
  const eventSpanHours = Math.max(1, Math.round((eventEnd - eventStart) / 3600_000));
  ...
  return <div className="propagation-layout">
    ...
    <section className="surface propagation-detail">{selected ? <>
      <div className="detail-heading"><div><p className="eyebrow">EVIDENCE-BASED PROPAGATION</p><h2>{selected.title}</h2><p>系统优先使用原文链接；否则结合发布时间顺序、标题和正文特征相似度推断最可能的复制来源。推断结果始终显示置信度与证据。</p></div><div className="chain-stat"><strong>{selected.countries.length}</strong><span>国家 / 地区</span><small>{selectedEdges.filter((item) => item.cross_border).length} 次跨境传播</small></div></div>
      <div className="event-analysis-strip"><div><span>爆发窗口</span><strong>{eventSpanHours < 24 ? `${eventSpanHours}h` : `${Math.round(eventSpanHours / 24)}d`}</strong><small>首发至最后转载</small></div><div><span>峰值时段</span><strong>+{peak[0]}h</strong><small>该小时新增 {peak[1]} 篇</small></div><div><span>高度转载率</span><strong>{reprintRatio}%</strong><small>文本相似度 ≥ 45%</small></div><div><span>平均推断置信</span><strong>{avgConfidence || "—"}</strong><small>{avgConfidence ? "可解释证据评分" : "等待形成传播边"}</small></div><div><span>传播速度</span><strong>{(selected.items.length / Math.max(1, eventSpanHours) * 24).toFixed(1)}</strong><small>篇 / 24 小时</small></div></div>
      ...
      <div className="route-strip">{selected.items.map((item, index) => <div key={item.id}><span>{countryCode[item.source_country] ?? "GL"}</span><strong>{item.source_country}</strong><small>{index === 0 ? "最早信源" : formatDate(item.published_at)}</small>{index < selected.items.length - 1 && <i>→</i>}</div>)}</div>
      <div className="chain-timeline">{selected.items.map((item, index) => { const edge = edgeByTarget.get(item.id); const parent = edge ? mentionById.get(edge.from_mention_id) : undefined; return <article key={item.id}>
        <div className={index === 0 ? "chain-node origin" : "chain-node"}>{String(index + 1).padStart(2, "0")}</div>
        <div className="chain-card"><div className="chain-meta"><span>{formatDate(item.published_at, true)}</span><span>{item.source_country}</span><span>{item.source}</span><span>{item.platform}</span></div><a href={item.url} target="_blank" rel="noreferrer">{item.title} ↗</a><p>{item.excerpt || item.summary}</p>{edge && <div className="edge-evidence"><div><b>{edge.method}</b><span>{edge.confidence}% 置信度</span><span>{edge.similarity}% 文本相似</span></div><p>推断来源：<strong>{parent?.source ?? "上游公开信源"}</strong> · 间隔 {gapLabel(edge.time_gap_minutes)} · {edge.evidence}</p></div>}</div>
      </article>; })}</div>
      <div className="event-comparison"><div className="section-head"><div><p className="eyebrow">CROSS-EVENT COMPARISON</p><h3>并列事件对比</h3></div><span className="subtle-note">气泡大小代表报道量</span></div><div className="comparison-layout"><div className="event-scatter"><span className="axis-label y">高风险 ↑</span><span className="axis-label x">传播速度 →</span>{clusters.slice(0, 12).map((cluster, index) => { const start = new Date(cluster.items[0].published_at).getTime(); const end = new Date(cluster.items.at(-1)!.published_at).getTime(); const hours = Math.max(1, (end - start) / 3600_000); const velocity = cluster.items.length / hours; const maxVelocity = Math.max(.01, ...clusters.slice(0, 12).map((item) => { const first = new Date(item.items[0].published_at).getTime(); const last = new Date(item.items.at(-1)!.published_at).getTime(); return item.items.length / Math.max(1, (last - first) / 3600_000); })); return <button key={cluster.key} className={cluster.key === selected.key ? "active" : ""} style={{ left: `${12 + velocity / maxVelocity * 76}%`, bottom: `${10 + cluster.risk / 100 * 75}%`, width: `${18 + Math.min(34, cluster.items.length * 4)}px`, height: `${18 + Math.min(34, cluster.items.length * 4)}px` }} title={`${cluster.title} · ${cluster.items.length} 篇 · 风险 ${cluster.risk}`} onClick={() => onSelect(cluster.key)}>{index + 1}</button>; })}</div><div className="comparison-table">{clusters.slice(0, 6).map((cluster, index) => <button key={cluster.key} onClick={() => onSelect(cluster.key)}><span>{index + 1}</span><div><strong>{cluster.title}</strong><small>{cluster.countries.length} 地区 · {cluster.items.length} 篇</small></div><b className={`risk-score ${riskClass(cluster.risk)}`}>{cluster.risk}</b></button>)}</div></div></div>
...
function CoverageView({ connectors, sources, submit }: { connectors: Connector[]; sources: MediaSource[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown> }) {
  ...
  async function saveCredential(event: FormEvent<HTMLFormElement>) {
    ...
    try { await submit({ action: "saveConnectorCredential", provider: editing, credential }, `${editing} API 已加密保存，将用于你的手动巡检`); setCredential(""); setEditing(null); }
  ...
  return <div className="coverage-page">
    <section className="coverage-architecture panel-dark"><div><p className="eyebrow">HYBRID COLLECTION ARCHITECTURE</p><h2>少量付费发现，<br /><em>大部分追踪免费完成。</em></h2><p>全球接口每 6 小时寻找新报道和新媒体；一旦发现媒体域名，系统会自动检查 robots.txt，并优先订阅其 RSS、Atom 或新闻 Sitemap。搜索额度用于“发现”，免费爬虫用于“持续跟踪”。</p></div><div className="architecture-flow"><article><b>01</b><strong>全球发现</strong><span>NewsAPI.ai / GDELT</span><small>每 6 小时 / 每日</small></article><i>→</i><article><b>02</b><strong>来源库</strong><span>{sources.length} 个媒体域名</span><small>自动增长</small></article><i>→</i><article><b>03</b><strong>免费追踪</strong><span>RSS / Atom / Sitemap</span><small>每小时轮询</small></article></div></section>
    <section className="surface connector-section"><div className="section-head"><div><p className="eyebrow">CONNECTOR STATUS</p><h3>采集连接器</h3></div></div><div className="connector-grid">{connectors.map((connector) => <article key={connector.id}><div><i className={connector.status} /><strong>{connector.name}</strong><span className={`connector-state ${connector.status}`}>{connector.status === "online" ? "运行中" : connector.status === "limited" ? "退避 / 建库中" : connector.status === "credentials" ? "待凭证" : "需授权"}</span></div><p>{connector.detail}</p>{connector.configurable && connector.provider && <button className="connector-config-button" onClick={() => { setEditing(connector.provider!); setCredential(""); }}>{connector.configured ? `已配置 · ${connector.lastFour === "环境密钥" ? "站点默认密钥" : `•••• ${connector.lastFour}`}` : "＋ 配置我的 API"}</button>}</article>)}</div></section>
    <section className="surface credential-vault"><div className="vault-copy"><p className="eyebrow">PERSONAL API VAULT</p><h3>我的数据连接器</h3><p>每位登录用户都可以保存自己的 NewsAPI.ai、X 和 YouTube 密钥。密钥在服务端使用 AES-GCM 加密，完整值不会返回浏览器；你的手动巡检优先使用自己的密钥，定时后台任务仍使用站点默认密钥。</p><div className="vault-security"><span>✓ 按登录用户隔离</span><span>✓ 服务端加密</span><span>✓ 前端仅显示末四位</span></div></div><div className="credential-list">{configurable.map((connector) => <article key={connector.id}><div><i className={connector.configured ? "configured" : ""} /><div><strong>{connector.name}</strong><small>{connector.configured ? connector.lastFour === "环境密钥" ? "当前使用站点默认密钥" : `个人密钥 •••• ${connector.lastFour}` : "尚未配置个人密钥"}</small></div></div><div><button onClick={() => setEditing(connector.provider!)}>{connector.configured ? "更换" : "配置"}</button>{connector.configured && connector.lastFour !== "环境密钥" && <button className="danger-link" disabled={busy} onClick={() => void removeCredential(connector.provider!)}>删除</button>}</div></article>)}</div></section>
    {editing && <div className="credential-modal-backdrop" onMouseDown={() => setEditing(null)}><form className="credential-modal" onSubmit={saveCredential} onMouseDown={(event) => event.stopPropagation()}><div className="section-head"><div><p className="eyebrow">SECURE CONNECTOR SETUP</p><h3>配置 {editing}</h3></div><button type="button" className="modal-close" onClick={() => setEditing(null)}>×</button></div><p>输入你自己的 {editing === "X" ? "Bearer Token" : "API Key"}。保存后仅服务端可以解密使用。</p><label className="field"><span>{editing === "X" ? "Bearer Token" : "API Key"}</span><input type="password" autoComplete="off" autoFocus required value={credential} onChange={(event) => setCredential(event.target.value)} placeholder="粘贴密钥" /></label><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setEditing(null)}>取消</button><button className="primary-button" disabled={busy || !credential.trim()}>{busy ? "加密保存中…" : "安全保存"}</button></div></form></div>}
...
function SettingsView({ brand, connectors, entities, submit }: { brand: BrandProfile; connectors: Connector[]; entities: Entity[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown> }) {
  ...
  return <div className="settings-grid">
    ...
    <aside className="surface automation-card"><div className="section-head"><div><p className="eyebrow">AUTOMATION POLICY</p><h3>自动运行策略</h3></div></div>{[["全球发现", "每 6 小时少量查询，用于发现新来源", true], ["GDELT 兜底", "每日最多一次，并遵守 429 退避", true], ["免费媒体追踪", "每小时轮询来源库，使用缓存验证", true], ["robots.txt", "不抓取禁止路径，不绕过验证码与付费墙", true], ["传播链路", "每次入库后重建可解释的传播证据", true], ["Meta / TikTok", "等待官方授权或合规供应商", false]].map(([title, note, on]) => <div className="policy-row" key={String(title)}><div><strong>{title}</strong><small>{note}</small></div><span className={on ? "toggle on" : "toggle"}><i /></span></div>)}<div className="connector-mini">{connectors.map((item) => <div key={item.id}><span>{item.name}</span><strong>{item.status === "online" ? "运行中" : item.status === "limited" ? "自动退避" : "待接入"}</strong></div>)}</div></aside>
  </div>;
}
