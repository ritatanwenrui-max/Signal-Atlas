...
type SocialCommentRow = {
  ...
  author_username: string; author_name: string; is_verified: number; content: string; sentiment: string; sentiment_score: number;
...
const emptyAnalytics: Analytics = { countries: [], sentiment: { positive: 0, neutral: 0, negative: 0, mixed: 0 }, timeline: [], words: [], commentWords: [], commentSentiment: { positive: 0, neutral: 0, negative: 0, mixed: 0 }, commentsAnalyzed: 0, sources: [], crossBorderEdges: 0, archivedTotal: 0 };
...
const nav = [
  ...
  ["analytics", "舆情分析", "04"], ["comments", "社媒评论", "05"], ["coverage", "来源覆盖", "06"], ["settings", "品牌配置", "07"],
] as const;
...
function discoveryLabel(value: string) { return value === "free_crawler" ? "免费追踪" : value === "official_api" ? "官方 API" : value === "monid_public_search" ? "Monid 公开搜索" : value === "manual" ? "人工证据" : "全球发现"; }
...
function metricLabel(value?: number) { return value != null && value >= 0 ? value.toLocaleString() : "未披露"; }
    ...
    try {
      ...
      if (announce) {
        ...
        const message = result.sync.skipped ? `系统刚完成过巡检，档案已是最新状态${commentNote}`
          : `巡检完成：共发现 ${result.sync.found} 条，新增归档 ${result.sync.inserted} 条，追踪 ${result.sync.crawledSources ?? 0} 个媒体源${result.sync.socialPending ? `；${result.sync.socialPending} 个 Instagram 任务正在后台处理` : ""}${commentNote}`;
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      {data.viewer.authenticated && <div className="system-card">
        ...
        <div className="system-row"><span>Instagram 搜索</span><strong>{monidConnector?.configured ? "每 6 小时" : "待配置"}</strong></div>
...
function ArchiveView({ mentions, allCount, countries, platforms, platformCounts, country, platform, sentiment, setCountry, setPlatform, setSentiment }: { mentions: Mention[]; allCount: number; countries: string[]; platforms: string[]; platformCounts: Record<string, number>; country: string; platform: string; sentiment: string; setCountry: (value: string) => void; setPlatform: (value: string) => void; setSentiment: (value: string) => void }) {
  ...
  function exportCsv() {
    const fields = [["发布时间", "标题", "链接", "媒体", "地区", "平台", "发帖人ID", "发帖人用户名", "粉丝数", "点赞数", "社媒评论数", "转发数", "播放数", "页面公开评论数", "实际分析评论数", "评论情绪", "评论关键词", "语言", "报道情绪", "风险分", "发现方式", "传播事件"], ...ordered.map((item) => [item.published_at, item.title, item.url, item.source, item.source_country, item.platform, item.social_author_id ?? "", item.social_author_username ?? "", metricLabel(item.social_follower_count), metricLabel(item.social_likes), metricLabel(item.social_comments), metricLabel(item.social_shares), metricLabel(Math.max(item.social_views ?? -1, item.social_plays ?? -1)), item.comment_reported_count ?? "", item.comment_analyzed_count ?? "", item.comment_sentiment ?? "", parsedCommentKeywords(item.comment_keywords).map((entry) => entry.word).join("、"), item.language, item.sentiment, item.risk, discoveryLabel(item.discovered_via), item.cluster_key])];
  ...
  return <div className="archive-page">
    ...
    <section className="surface archive-table-card">
      ...
      <div className="table-scroll"><table className="archive-table"><thead><tr><th>发布时间</th><th>新闻 / 社媒原文</th><th>媒体 / 账号</th><th>互动 / 评论舆情</th><th>地区</th><th>平台</th><th>语言</th><th>情绪</th><th>风险</th><th>发现方式</th><th>事件</th></tr></thead><tbody>{rows.map((item) => { const commentWords = parsedCommentKeywords(item.comment_keywords).slice(0, 5); return <tr key={item.id}><td className="date-cell">{formatDate(item.published_at, true)}</td><td className="title-cell"><a href={item.url} target="_blank" rel="noreferrer">{item.title}<span>↗</span></a><small>{item.excerpt || item.summary}</small></td><td><strong>{item.source}</strong>{item.author && item.author !== item.source && <small>{item.author}</small>}</td><td className="social-metrics-cell">{item.platform === "Instagram" ? <><strong>{item.social_author_username ? `@${item.social_author_username}` : item.author || "账号待补全"}</strong><small>ID {item.social_author_id || "待补全"} · 粉丝 {item.social_follower_count ? item.social_follower_count.toLocaleString() : "补全中"}</small><small>赞 {metricLabel(item.social_likes)} · 评 {metricLabel(item.social_comments)} · 转 {metricLabel(item.social_shares)} · 播 {metricLabel(Math.max(item.social_views ?? -1, item.social_plays ?? -1))}</small></> : item.comment_status === "collected" ? <><strong>公开 {item.comment_reported_count ?? 0} · 已分析 {item.comment_analyzed_count ?? 0}</strong><small>评论情绪：<b className={sentimentClass(item.comment_sentiment ?? "中性")}>{item.comment_sentiment}</b> · 净值 {item.comment_sentiment_score ?? 0}</small><small>{commentWords.length ? `关键词：${commentWords.map((entry) => entry.word).join(" · ")}` : "样本中暂无高频关键词"}</small></> : item.comment_reported_count ? <><strong>页面显示 {item.comment_reported_count} 条评论</strong><small>{item.comment_last_error || "评论文本暂不可读取，未生成情绪结论"}</small></> : item.comment_status ? <><strong>{item.comment_status === "empty" ? "暂无公开评论" : item.comment_status === "error" ? "评论采集失败" : "未发现可读评论"}</strong><small>{item.comment_last_error || `${item.comment_adapter || "公开评论适配器"}已检查`}</small></> : <><strong>等待评论巡检</strong><small>仅对实际取得的公开评论生成情绪和关键词</small></>}</td><td><span className="country-tag">{countryCode[item.source_country] ?? "GL"}</span>{item.source_country}<small title={item.location_method || "来源字段"}>{item.location_confidence ? `${item.location_method} · ${item.location_confidence}%` : "来源字段"}</small></td><td>{item.platform}</td><td>{item.language}</td><td><span className={`sentiment-pill ${sentimentClass(item.sentiment)}`}>{item.sentiment}</span></td><td><span className={`risk-score ${riskClass(item.risk)}`}>{item.risk}</span></td><td><span className="discovery-badge">{discoveryLabel(item.discovered_via)}</span></td><td><code>{item.cluster_key.replace("story-", "#")}</code></td></tr>; })}</tbody></table></div>
...
function SocialCommentsView({ brand, monidConfigured }: { brand: BrandProfile; monidConfigured: boolean }) {
  ...
  return <div className="comments-page">
    ...
    {!monidConfigured && <section className="comment-callout surface"><strong>评论采集尚未启动</strong><p>先在“来源覆盖”中配置 Monid API Key。配置后，系统会为所有已发现的 Instagram 相关帖子建立评论及回复分页任务。</p></section>}
...
function CoverageView({ connectors, sources, submit }: { connectors: Connector[]; sources: MediaSource[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown> }) {
  ...
  return <div className="coverage-page">
    <section className="coverage-architecture panel-dark"><div><h2>采集计划</h2><p>新闻发现与免费来源持续归档；配置 Monid 后，每 6 小时搜索 Instagram 公开帖子，并在后台分页归档帖子下的公开评论和回复。</p></div><div className="architecture-flow"><article><b>01</b><strong>全球与社媒发现</strong><span>NewsAPI.ai / GDELT / Monid</span><small>每 6 小时 / 每日</small></article><i>→</i><article><b>02</b><strong>来源库</strong><span>{sources.length} 个媒体域名</span><small>自动更新</small></article><i>→</i><article><b>03</b><strong>持续追踪</strong><span>新闻 / 帖子 / 评论回复</span><small>后台异步归档</small></article></div></section>
    ...
    {editing && <div className="credential-modal-backdrop" onMouseDown={() => setEditing(null)}><form className="credential-modal" onSubmit={saveCredential} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <p>{editing === "Monid / Instagram" ? "用于按品牌名、别名和扩展词搜索 Instagram 公开帖子，并分页归档所有可公开取得的评论及楼中楼回复。保存时会先验证密钥。" : editing === "Meta / Instagram" ? "用于采集 Business / Creator 账号的标签与 @提及；启用仍取决于 Meta 权限和 App Review。" : editing === "TikTok" ? "用于 TikTok Research API 的公开内容查询；启用仍取决于 Research API 审批。" : `输入你自己的 ${editing === "X" ? "Bearer Token" : "API Key"}。`} 保存后仅服务端可以解密使用。</p>
...
function SettingsView({ brand, connectors, entities, submit }: { brand: BrandProfile; connectors: Connector[]; entities: Entity[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown> }) {
  ...
  return <div className="settings-grid">
    ...
    <aside className="surface automation-card"><div className="section-head"><div><p className="eyebrow">AUTOMATION POLICY</p><h3>自动运行策略</h3></div></div>{[["调度巡检", "Cloudflare 每小时第 17 分钟触发", true], ["全球发现", "NewsAPI.ai 每 6 小时；GDELT 每日兜底", true], ["Instagram 公开搜索", monidConfigured ? "Monid 每 6 小时按普通文字关键词搜帖；异步任务完成后自动回收结果" : "在来源覆盖页配置 Monid API Key 后启用", monidConfigured], ["社媒评论与回复", monidConfigured ? "为全部相关 Instagram 帖子建立游标分页队列，持续归档公开评论和楼中楼回复" : "配置 Monid 后启用；仅保存实际取得的公开文本", monidConfigured], ["网页评论分析", "每 6 小时检查近期新闻；仅分析实际取得的公开评论文本", true], ["免费媒体追踪", "活跃源 3 小时、待探测源 12 小时，按到期批次抓取", true], ["robots.txt", "不抓取禁止路径，不绕过验证码与付费墙", true], ["传播链路", "先按时间与文本聚为同一事件，再重建有向传播图", true], ["Meta / TikTok 官方接口", officialSocialConfigured ? "凭证已保存，等待平台权限审核后启用采集" : "可选配置，不影响 Monid 的 Instagram 公开搜索", officialSocialConfigured]].map(([title, note, on]) => <div className="policy-row" key={String(title)}><div><strong>{title}</strong><small>{note}</small></div><span className={on ? "toggle on" : "toggle"}><i /></span></div>)}<div className="connector-mini">{connectors.map((item) => <div key={item.id}><span>{item.name}</span><strong>{item.status === "limited" ? "暂缓重试" : item.pending ? "采集中" : item.status === "online" ? "运行中" : item.configured ? "凭证已存" : "待接入"}</strong></div>)}</div></aside>
  </div>;
}
