...
function CoverageView({ connectors, sources, submit, canManage }: { connectors: Connector[]; sources: MediaSource[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canManage: boolean }) {
  ...
  return <div className="coverage-page">
    ...
    {editing && canManage && <div className="credential-modal-backdrop" onMouseDown={() => setEditing(null)}><form className="credential-modal" onSubmit={saveCredential} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <label className="field"><span>{editing === "Monid / Instagram" ? "Monid API Key" : editing === "Meta / Instagram" ? "Long-lived Access Token" : editing === "TikTok" ? "Client Key" : editing === "Azure Translator" ? "Azure Translator Key" : editing === "LibreTranslate" ? "LibreTranslate HTTPS 地址" : editing === "MyMemory" ? "联系邮箱" : editing === "X" ? "Bearer Token" : "API Key"}</span><input type={editing === "LibreTranslate" ? "url" : editing === "MyMemory" ? "email" : "password"} autoComplete="off" autoFocus={editing !== "Meta / Instagram"} required value={credential} onChange={(event) => setCredential(event.target.value)} placeholder={editing === "Monid / Instagram" ? "monid_live_…" : editing === "LibreTranslate" ? "https://translate.example.com" : editing === "MyMemory" ? "name@company.com" : "粘贴凭证"} /></label>
