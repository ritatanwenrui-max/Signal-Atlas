...
type Mention = {
  ...
  translation_en: string; translation_status: string; translation_provider: string; translated_at: string;
...
function EnglishTranslation({ value, status }: { value?: string; status?: string }) {
  if (value?.trim()) return <div className="english-translation"><span>EN</span><p>{value.trim()}</p></div>;
  const label = status === "translating" ? "Translating…" : status === "blocked" ? "Translation paused: check Monid balance or access"
    : status === "error" ? "Translation will retry automatically" : "English translation queued";
...
function ArchiveView({ mentions, allCount, countries, platforms, platformCounts, country, platform, sentiment, setCountry, setPlatform, setSentiment, submit, canEdit }: { mentions: Mention[]; allCount: number; countries: string[]; platforms: string[]; platformCounts: Record<string, number>; country: string; platform: string; sentiment: string; setCountry: (value: string) => void; setPlatform: (value: string) => void; setSentiment: (value: string) => void; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canEdit: boolean }) {
  ...
  function exportCsv() {
    ...
    const fields = [["发布时间", "平台", "原文", "英文翻译", "链接", "媒体/账号", "地区", "点赞", "评论", "转发", "播放", "已分析评论", "极性", "具体情绪", "风险分", "传播事件"], ...ordered.map((item) => [item.published_at, item.platform, [item.title, item.excerpt || item.summary].filter(Boolean).join("\n"), item.translation_en, item.url, item.source, item.source_country, visible(item.social_likes), visible(item.social_comments), visible(item.social_shares), visible(Math.max(item.social_views ?? -1, item.social_plays ?? -1)), item.comment_analyzed_count ?? "", item.sentiment, item.emotion, item.risk, item.cluster_key])];
  ...
  return <div className="archive-page">
    ...
    <section className="surface archive-table-card">
      ...
      <div className="table-scroll"><table className="archive-table"><thead><tr><th>发布时间</th><th>平台</th><th>新闻 / 社媒原文</th><th>媒体 / 账号</th><th>互动</th><th>地区</th><th>具体情绪</th><th>风险</th><th>事件</th></tr></thead><tbody>{rows.map((item) => <tr key={item.id}><td className="date-cell">{formatDate(item.published_at, true)}</td><td><span className={`platform-badge platform-${item.platform.toLowerCase().replace("网页新闻", "web")}`}>{item.platform}</span></td><td className="title-cell"><a href={item.url} target="_blank" rel="noreferrer">{item.title}<span>↗</span></a><small>{item.excerpt || item.summary}</small><EnglishTranslation value={item.translation_en} status={item.translation_status} /></td><td><strong>{item.source}</strong>{item.author && item.author !== item.source && <small>{item.author}</small>}</td><td>{interaction(item)}</td><td><span className="country-tag">{countryCode[item.source_country] ?? "GL"}</span>{item.source_country}</td><td><span className="emotion-pill">{item.emotion || item.sentiment}</span><small>{item.sentiment}</small></td><td><span className={`risk-score ${riskClass(item.risk)}`}>{item.risk}</span></td><td><code>{item.cluster_key.replace("story-", "#")}</code></td></tr>)}</tbody></table></div>
...
function SocialCommentsView({ brand, monidConfigured, canEdit }: { brand: BrandProfile; monidConfigured: boolean; canEdit: boolean }) {
  ...
  return <div className="comments-page">
    ...
    <section className="surface risk-queue"><div className="section-head"><div><p className="eyebrow">RISK REVIEW QUEUE</p><h3>负面与混合情绪复核</h3></div><span className="subtle-note">按情绪分与互动量排序</span></div><div>{data?.riskComments.map((comment) => <article key={comment.id}><header><span className={`sentiment-pill ${sentimentClass(comment.sentiment)}`}>{comment.sentiment}</span><strong>{comment.likes} 赞</strong></header><p>{comment.content}</p><EnglishTranslation value={comment.translation_en} status={comment.translation_status} /><a href={comment.post_url} target="_blank" rel="noreferrer">{comment.post_title} ↗</a></article>)}{!data?.riskComments.length && <div className="comment-empty compact">暂无需要复核的高风险评论</div>}</div></section>
