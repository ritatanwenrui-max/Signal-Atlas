...
function EnglishTranslation({ value, status, language, error, nextRetryAt }: {
  value?: string; status?: string; language?: string; error?: string; nextRetryAt?: string;
}) {
...
function ArchiveView({ mentions, allCount, countries, platforms, platformCounts, country, platform, sentiment, setCountry, setPlatform, setSentiment, submit, canEdit }: { mentions: Mention[]; allCount: number; countries: string[]; platforms: string[]; platformCounts: Record<string, number>; country: string; platform: string; sentiment: string; setCountry: (value: string) => void; setPlatform: (value: string) => void; setSentiment: (value: string) => void; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canEdit: boolean }) {
  ...
  function exportCsv() {
    ...
    const fields = [["发布时间", "平台", "原文", "英文翻译", "链接", "媒体/账号", "地区", "语言", "地区判断依据", "地区置信度", "点赞", "评论", "转发", "播放", "已分析评论", "极性", "具体情绪", "风险分", "传播事件"], ...ordered.map((item) => [item.published_at, item.platform, [item.title, item.excerpt || item.summary].filter(Boolean).join("\n"), translationNotNeeded(item.language) ? "" : item.translation_en, item.url, item.source, item.source_country, item.language, item.location_method, item.location_confidence, visible(item.social_likes), visible(item.social_comments), visible(item.social_shares), visible(Math.max(item.social_views ?? -1, item.social_plays ?? -1)), item.comment_analyzed_count ?? "", item.sentiment, item.emotion, item.risk, item.cluster_key])];
  ...
  return <div className="archive-page">
    ...
    <section className="surface reddit-archive-status"><div><span className="platform-badge platform-reddit">Reddit</span><div><strong>Reddit 公开讨论已纳入媒体档案</strong><p>按品牌关键词发现帖子，随后补全帖子正文、subreddit、作者、赞同数和评论数，并继续采集公开评论与嵌套回复。</p></div></div><button type="button" className="secondary-button" onClick={() => { setPlatform("Reddit"); setPage(1); }}>查看 Reddit（{platformCounts.Reddit ?? 0}）</button></section>
