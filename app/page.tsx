...
type Connector = { id: string; name: string; provider?: string; configurable?: boolean; configured?: boolean; lastFour?: string; status: "online" | "limited" | "credentials" | "approval"; detail: string; retryAt?: string; pending?: number; lastError?: string;
  quotaUsed?: number; quotaLimit?: number; quotaRemaining?: number; quotaResetAt?: string; scheduleLabel?: string };
...
function CoverageView({ connectors, sources, diagnostics, submit, canManage }: { connectors: Connector[]; sources: MediaSource[]; diagnostics: CollectionDiagnostic[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown>; canManage: boolean }) {
  ...
  return <div className="coverage-page">
    <section className="coverage-architecture panel-dark"><div><h2>采集计划与混合分析</h2><p>新闻档案优先使用多个独立新闻源和社交搜索索引发现新内容；搜索与归档确认没有新增后，后台才进入帖子评论、回复和受众分析阶段。</p></div><div className="architecture-flow"><article><b>01</b><strong>全球新闻发现</strong><span>NewsAPI.ai / Media Cloud / NewsData / World News / GDELT</span><small>按各自额度分时运行</small></article><i>→</i><article><b>02</b><strong>社交内容补漏</strong><span>ScrapeCreators / Brave / Apify / Bright Data</span><small>平台搜索与多索引交叉发现</small></article><i>→</i><article><b>03</b><strong>规则全量分析</strong><span>品牌限定 / 排除词 / 地区 / 事件</span><small>人工标注优先</small></article><i>→</i><article><b>04</b><strong>受众采集</strong><span>评论 / 回复 / 翻译 / 语义</span><small>档案阶段完成后运行</small></article></div></section>
    ...
    {editing && canManage && <div className="credential-modal-backdrop" onMouseDown={() => setEditing(null)}><form className="credential-modal" onSubmit={saveCredential} onMouseDown={(event) => event.stopPropagation()}>
      ...
      {editing === "Google Search Console" ? <><label className="field"><span>Search Console 资源 *</span><input autoComplete="off" autoFocus required value={secondaryCredential} onChange={(event) => setSecondaryCredential(event.target.value)} placeholder="sc-domain:example.com" /></label><label className="field"><span>服务账号或 OAuth JSON *</span><textarea rows={9} autoComplete="off" required value={credential} onChange={(event) => setCredential(event.target.value)} placeholder={'{\n  "type": "service_account",\n  "client_email": "…",\n  "private_key": "…"\n}'} /></label></> : <label className="field"><span>{editing === "Monid / Instagram" ? "Monid API Key" : editing === "OpenAI LLM" ? "OpenAI API Key" : editing === "Meta / Instagram" ? "Long-lived Access Token" : editing === "TikTok" ? "Client Key" : editing === "Apify" ? "API Token" : editing === "Azure Translator" ? "Azure Translator Key" : editing === "LibreTranslate" ? "LibreTranslate HTTPS 地址" : editing === "MyMemory" ? "联系邮箱" : editing === "X" ? "Bearer Token" : "API Key"}</span><input type={editing === "LibreTranslate" ? "url" : editing === "MyMemory" ? "email" : "password"} autoComplete="off" autoFocus={editing !== "Meta / Instagram"} required value={credential} onChange={(event) => setCredential(event.target.value)} placeholder={editing === "Monid / Instagram" ? "monid_live_…" : editing === "OpenAI LLM" ? "sk-…" : editing === "LibreTranslate" ? "https://translate.example.com" : editing === "MyMemory" ? "name@company.com" : "粘贴凭证"} /></label>}
      ...
      <div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setEditing(null)}>取消</button><button className="primary-button" disabled={busy || !credential.trim() || (editing === "Meta / Instagram" && !accountId.trim()) || (["TikTok", "Bright Data", "Google Search Console"].includes(editing) && !secondaryCredential.trim())}>{busy ? "保存中…" : "安全保存"}</button></div>
    </form></div>}
