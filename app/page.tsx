    ...
    try {
      ...
      if (announce) {
        const message = result.sync.skipped ? "系统刚完成过巡检，档案已是最新状态"
          : `巡检完成：共发现 ${result.sync.found} 条，新增归档 ${result.sync.inserted} 条，追踪 ${result.sync.crawledSources ?? 0} 个媒体源${result.sync.socialPending ? `；${result.sync.socialPending} 个 Instagram 任务正在后台处理` : ""}`;
...
function ArchiveView({ mentions, allCount, countries, platforms, platformCounts, country, platform, sentiment, setCountry, setPlatform, setSentiment }: { mentions: Mention[]; allCount: number; countries: string[]; platforms: string[]; platformCounts: Record<string, number>; country: string; platform: string; sentiment: string; setCountry: (value: string) => void; setPlatform: (value: string) => void; setSentiment: (value: string) => void }) {
  ...
  function exportCsv() {
    const fields = [["发布时间", "标题", "链接", "媒体", "地区", "平台", "发帖人ID", "发帖人用户名", "粉丝数", "点赞数", "评论数", "转发数", "播放数", "语言", "情绪", "风险分", "发现方式", "传播事件"], ...ordered.map((item) => [item.published_at, item.title, item.url, item.source, item.source_country, item.platform, item.social_author_id ?? "", item.social_author_username ?? "", metricLabel(item.social_follower_count), metricLabel(item.social_likes), metricLabel(item.social_comments), metricLabel(item.social_shares), metricLabel(Math.max(item.social_views ?? -1, item.social_plays ?? -1)), item.language, item.sentiment, item.risk, discoveryLabel(item.discovered_via), item.cluster_key])];
...
function SettingsView({ brand, connectors, entities, submit }: { brand: BrandProfile; connectors: Connector[]; entities: Entity[]; submit: (payload: Record<string, unknown>, success: string) => Promise<unknown> }) {
  ...
  return <div className="settings-grid">
    ...
    <aside className="surface automation-card"><div className="section-head"><div><p className="eyebrow">AUTOMATION POLICY</p><h3>自动运行策略</h3></div></div>{[["调度巡检", "Cloudflare 每小时第 17 分钟触发", true], ["全球发现", "NewsAPI.ai 每 6 小时；GDELT 每日兜底", true], ["Instagram 公开搜索", monidConfigured ? "Monid 每 6 小时按普通文字关键词搜帖，并异步补全作者数据" : "在来源覆盖页配置 Monid API Key 后启用", monidConfigured], ["免费媒体追踪", "活跃源 3 小时、待探测源 12 小时，按到期批次抓取", true], ["robots.txt", "不抓取禁止路径，不绕过验证码与付费墙", true], ["传播链路", "先按时间与文本聚为同一事件，再重建有向传播图", true], ["Meta / TikTok 官方接口", officialSocialConfigured ? "凭证已保存，等待平台权限审核后启用采集" : "可选配置，不影响 Monid 的 Instagram 公开搜索", officialSocialConfigured]].map(([title, note, on]) => <div className="policy-row" key={String(title)}><div><strong>{title}</strong><small>{note}</small></div><span className={on ? "toggle on" : "toggle"}><i /></span></div>)}<div className="connector-mini">{connectors.map((item) => <div key={item.id}><span>{item.name}</span><strong>{item.status === "online" ? "运行中" : item.status === "limited" ? "自动退避" : item.configured ? "凭证已存" : "待接入"}</strong></div>)}</div></aside>
  </div>;
}
